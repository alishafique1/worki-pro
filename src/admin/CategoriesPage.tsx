...
export default function AdminCategoriesPage() {
  ...
  const categories = rawCats as Category[] | undefined;
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={openCreate}
          className="px-5 py-2.5 bg-[#2563EB] text-white text-sm font-bold rounded-[12px] hover:bg-[#1D4ED8] transition-colors shadow-[0_4px_12px_rgba(37,99,235,0.3)]"
        >
          + Add Category
        </button>
      </div>
      ...
      {isLoading && (
        <div className="space-y-3">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="animate-pulse h-16 bg-[#F8FAFC] rounded-[16px] border border-[#E2E8F0]" />
          ))}
        </div>
      )}
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-[20px] overflow-hidden shadow-sm">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {categories?.map((cat) => (
                <tr key={cat.id} className="border-b border-[#E2E8F0] last:border-0 hover:bg-[#F8FAFC] transition-colors">
                  <td className="px-5 py-4">
                    <div className="font-semibold text-[#0F172A]">{cat.name}</div>
                    {cat.description && (
                      <div className="text-xs text-[#475569] mt-0.5 line-clamp-1">{cat.description}</div>
                    )}
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-bold border border-[#E2E8F0] text-[#475569] rounded-[8px] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => setConfirmDelete(cat)}
                        className="px-3 py-1.5 text-xs font-bold border border-red-200 text-red-500 rounded-[8px] hover:bg-red-50 transition-colors"
                      >
                        Delete
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[24px] border border-[#E2E8F0] shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              {errMsg && (
                <div className="rounded-[12px] bg-red-500/10 border border-red-400/30 px-4 py-3 text-sm text-red-600">{errMsg}</div>
              )}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-[#E2E8F0] rounded-[12px] px-4 py-2.5 text-sm text-[#0F172A] font-mono focus:outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/20"
                  placeholder="e.g. food-catering"
                />
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-[#E2E8F0] rounded-[12px] px-4 py-2.5 text-sm text-[#0F172A] focus:outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/20 bg-white"
                >
              ...
              <div>
                ...
                />
                {form.imageUrl && (
                  <img src={form.imageUrl} alt="" className="mt-2 h-20 w-full object-cover rounded-[8px]" onError={(e) => (e.currentTarget.style.display = 'none')} />
                )}
              </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${form.active ? 'bg-[#2563EB]' : 'bg-[#E2E8F0]'}`}
                >
            ...
            <div className="p-6 border-t border-[#E2E8F0] flex gap-3">
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 py-3 bg-[#2563EB] text-white font-bold rounded-[12px] hover:bg-[#1D4ED8] transition-colors disabled:opacity-50 shadow-[0_4px_12px_rgba(37,99,235,0.3)]"
              >
                {saving ? 'Saving…' : form.id ? 'Save Changes' : 'Create Category'}
              </button>
              <button
                onClick={() => setForm(null)}
                className="px-6 py-3 border border-[#E2E8F0] text-[#475569] font-bold rounded-[12px] hover:bg-[#F8FAFC] transition-colors"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="bg-white rounded-[24px] border border-[#E2E8F0] shadow-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 bg-red-600 text-white font-bold rounded-[12px] hover:bg-red-700 transition-colors disabled:opacity-50"
              >
