...
const FILTER_TABS = [
  { label: 'All', statuses: null as string[] | null },
...
export default function AdminRequestsPage() {
  ...
  const counts = useMemo(() => {
    ...
    for (const r of allRequests) {
      if (NEW_STATUSES.has(r.status)) newCount++;
      else if (IN_PROGRESS_STATUSES.has(r.status)) inProgress++;
      else if (COMPLETED_STATUSES.has(r.status)) completed++;
      else if (LOST_STATUSES.has(r.status)) lost++;
    }
  ...
  const filtered = useMemo(() => {
    return allRequests.filter((req) => {
      if (activeTab.statuses && !(activeTab.statuses as string[]).includes(req.status)) return false;
      if (search) {
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative w-full sm:w-80">
            ...
            <input
              type="text"
              placeholder="Search description, name, city…"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-9 pr-3 py-2.5 text-sm rounded-[12px] bg-white border border-[#E2E8F0] text-[#0F172A] placeholder:text-[#94A3B8] outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10 transition-all"
            />
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                      ...
                      <div className="flex flex-col items-center justify-center py-16 text-center">
                        ...
                        {(search || filter !== 'All') && (
                          <button
                            ...
                            className="mt-4 px-4 py-2 rounded-[14px] bg-[#2563EB] text-white text-sm font-semibold hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.25)]"
                          >
                            Clear filters
                          </button>
                        )}
                ...
                {!reqLoading && filtered.map((req: any) => {
                  ...
                  return (
                    <tr
                      key={req.id}
                      className="group border-b border-[#E2E8F0] last:border-0 border-l-2 border-l-transparent hover:border-l-[#2563EB] hover:bg-[#F8FAFC] transition-colors align-middle"
                    >
                      {/* Status */}
                      ...
                      <td className="py-4 pr-3 max-w-[260px]">
                        <div className="flex items-center gap-2">
                          {req.urgency && (
                            <span
                              title={req.urgency}
                              className={`h-2 w-2 shrink-0 rounded-full ${URGENCY_DOT[req.urgency] ?? URGENCY_DOT.STANDARD}`}
                            />
                          )}
                        ...
                        <div className="flex flex-col items-end gap-1.5">
                          {isAssigning ? (
                            <div className="flex items-center gap-2">
                              <div className="relative">
                                <select
                                  ...
                                  onChange={(e) =>
                                    setSelectedProviderId((prev) => ({ ...prev, [req.id]: e.target.value }))
                                  }
                                  className="appearance-none text-sm rounded-[12px] bg-white border border-[#E2E8F0] text-[#0F172A] pl-3 pr-8 py-1.5 outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10 transition-all"
                                >
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 text-sm bg-[#2563EB] hover:bg-[#1D4ED8] text-white px-3 py-1.5 rounded-[14px] font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.25)]"
                              >
                              ...
                              <button
                                ...
                                className="text-sm font-medium text-[#475569] hover:text-[#0F172A] px-2 py-1.5"
                              >
                                Cancel
                              </button>
                            </div>
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#2563EB] border border-[#E2E8F0] hover:border-[#2563EB] hover:bg-[#EFF6FF] px-3 py-1.5 rounded-[14px] transition-colors"
                            >
                          ...
                          {errorMap[req.id] && (
                            <p className="text-xs font-medium text-[#B91C1C]">{errorMap[req.id]}</p>
                          )}
                        </div>
                      </td>
                    </tr>
...
function SummaryCard({
  ...
  return (
    ...
    >
      ...
      <p
        className={`mt-2 text-4xl font-black tracking-tight ${accent ? 'text-white' : 'text-[#0F172A]'}`}
