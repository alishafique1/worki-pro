...
function getDefaultFaqs(categoryName: string) {
  return [
    {
...
export default function ServiceCategoryPage() {
  ...
  const dbChildren = useMemo(() => {
    if (!dbCategories || !dbCategory) return [];
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#F8FAFC] font-sans">
        ...
        <section
          className="relative bg-white pt-10 pb-12 sm:pt-14 sm:pb-16 border-b border-[#E2E8F0]"
        ...
        >
          <Container>
            ...
            <div className="flex items-start gap-6">
              {/* Icon */}
              {category.icon && (
                <div className="hidden sm:flex shrink-0 size-16 items-center justify-center rounded-2xl bg-[#EFF6FF] border border-[#BFDBFE]">
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <Link
                to={`/request-service?category=${category.slug}`}
        ...
        {category.subCategories.length === 0 && dbChildren.length > 0 && (
          <section className="py-14 sm:py-18 bg-[#F8FAFC]">
            <Container>
              <h2 className="text-2xl font-bold text-[#0F172A] sm:text-3xl mb-2">
                What we help with
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {dbChildren.map((child) => (
                  <div
                    key={child.id}
                    className="group flex flex-col gap-2 rounded-xl border border-[#E2E8F0] bg-[#EFF6FF] p-4 transition duration-150 hover:border-[#BFDBFE] hover:shadow-sm"
                  >
                    ...
                    {child.description && (
                      <p className="text-xs leading-5 text-[#475569]">{child.description}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <Container>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {SERVICE_AREAS.map((area) => (
                <Link
                  key={area.slug}
                  to={`/services/${category.slug}/${area.slug}`}
                  className="flex items-center gap-3 p-4 rounded-xl border border-[#E2E8F0] bg-white hover:border-[#BFDBFE] hover:bg-[#EFF6FF] transition group"
                >
