...
export default function OnboardingPage() {
  ...
  function validate(): string | null {
    ...
    if (step === 2) {
      ...
      if (!isValidCanadianPhone(form.phone)) return 'Enter a valid Canadian phone number (e.g. (416) 555-0100).';
      if (!form.postalCode.trim()) return 'Postal code is required.';
      if (!isValidCanadianPostal(form.postalCode)) return 'Enter a valid Canadian postal code (e.g. L9T 2X5).';
      if (!isGtaPostal(form.postalCode)) return 'We currently serve the GTA (postal codes starting with L or M). Other areas coming soon.';
    }
  ...
  async function submitOnboarding(overrides?: { interestCategoryIds?: string[] }): Promise<boolean> {
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-6">
        <div className="w-full max-w-md text-center">
          ...
          <div className="grid grid-cols-3 gap-3 mb-8">
            {[
              ...
              <a
                key={href}
                href={href}
                className="bg-white border border-[#E2E8F0] rounded-[20px] p-5 text-left hover:border-[#2563EB] hover:shadow-md transition-all group"
              >
          ...
          <a
            href="/get-quotes"
            className="inline-block px-8 py-3.5 bg-[#2563EB] text-white font-bold rounded-[22px] shadow-[0_8px_24px_rgba(37,99,235,0.3)] hover:bg-[#1D4ED8] transition-colors"
          >
            Find a local pro →
          </a>
        </div>
              ...
              {stepLabels.map((label, i) => {
                ...
                return (
                  <li key={num} className="flex items-start gap-3.5">
                    <div className={`size-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 text-xs font-black transition-all ${
                      isDone
          ...
          <div className="w-full max-w-lg">
              ...
              {Array.from({ length: totalSteps }).map((_, i) => (
                <div
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    i + 1 === step ? 'w-8 bg-[#2563EB]' : i + 1 < step ? 'w-4 bg-[#22C55E]' : 'w-4 bg-[#E2E8F0]'
                  }`}
                />
              ))}
            </div>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[24px] p-6 lg:p-8 shadow-sm">
              <div key={step} className="animate-in fade-in slide-in-from-right-3 duration-250">
                {step === 1 && (
                  <StepRole selected={form.role} onSelect={(role) => update('role', role)} />
                )}
                {step === 2 && (
                  <StepProfile
                    form={{
                ...
                {step === 3 && !isProvider && (
                  <CategoryCardGrid
                    ...
                    subtitle="Pick what you might need. We personalise your dashboard. (Optional)"
              ...
              {/* Error */}
              {error && (
                <div role="alert" className="mt-5 text-sm font-medium text-red-700 bg-red-50 border border-red-200 rounded-[12px] px-4 py-3 flex items-start gap-2">
              ...
              <div className="flex items-center justify-between mt-7 pt-5 border-t border-[#F1F5F9]">
                {step > 1 ? (
                  <button
                    type="button"
                    onClick={goBack}
                    className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-[12px] text-sm font-semibold text-[#475569] hover:text-[#0F172A] hover:bg-[#F8FAFC] border border-[#E2E8F0] transition-all"
                  >
                ...
                <div className="flex flex-col items-end gap-2">
                  <button
                    ...
                    className="px-7 py-2.5 bg-[#2563EB] text-white text-sm font-bold rounded-[14px] shadow-[0_4px_16px_rgba(37,99,235,0.35)] hover:bg-[#1D4ED8] active:scale-[0.98] transition-all disabled:opacity-50"
                  >
                    {isSubmitting
                      ? 'Saving…'
                      : step === totalSteps
                  ...
                  {step === 3 && !isProvider && (
                    <button
                      ...
                      disabled={isSubmitting}
                      className="text-xs text-[#94A3B8] hover:text-[#475569] transition-colors disabled:opacity-50"
                    >
                      Skip for now
                    </button>
                  )}
