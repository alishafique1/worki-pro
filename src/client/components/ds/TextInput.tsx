...
export const TextInput = React.forwardRef<HTMLInputElement, React.ComponentProps<'input'>>(
  ({ className, ...props }, ref) => (
    <input
      ref={ref}
      className={cn(
        'w-full bg-white border border-[#E2E8F0] rounded-control px-4 py-3 text-navy placeholder:text-muted-fg transition-colors focus:outline-none focus:border-brand focus:ring-2 focus:ring-[#2563EB]/30',
        className,
      )}
      {...props}
    />
