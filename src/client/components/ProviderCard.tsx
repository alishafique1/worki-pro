...
function VerificationBadge() {
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#DCFCE7] text-[#22C55E] text-xs font-semibold border border-[#BBF7D0]">
...
function CategoryBadge({ category }: { category: ProviderCategory }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-[#EFF6FF] text-[#2563EB] text-xs font-medium rounded-full border border-[#BFDBFE]">
      {icon}
...
export function ProviderCard({
  ...
  return (
    ...
    >
      {/* Featured badge */}
      {rank === 1 && (
        <div className="bg-[#2563EB] text-white text-xs font-bold px-4 py-1.5 text-center uppercase tracking-wider">
          #1 Top Rated
        </div>
      )}
      ...
      <div className="p-6">
        ...
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="shrink-0">
            {profilePhotoUrl ? (
              <img
                src={profilePhotoUrl}
                alt={businessName}
                className="w-14 h-14 rounded-full object-cover border-2 border-[#E2E8F0] group-hover:border-[#BFDBFE] transition-colors"
              />
            ) : (
              <div className="w-14 h-14 rounded-full bg-[#EFF6FF] border-2 border-[#BFDBFE] flex items-center justify-center group-hover:bg-[#DBEAFE] transition-colors">
          ...
          <div className="flex-1 min-w-0">
            ...
            {contactName && (
              <p className="text-sm text-[#475569] truncate mb-1">{contactName}</p>
            )}
        ...
        <div className="flex items-center gap-4 mb-4">
          <div className="flex items-center gap-2">
            <StarRating rating={ratingInternal} />
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {categories.slice(0, 3).map((pc) => (
            <CategoryBadge key={pc.serviceCategory.id} category={pc} />
          ))}
          {categories.length > 3 && (
            <span className="px-2.5 py-1 bg-[#F8FAFC] text-[#94A3B8] text-xs font-medium rounded-full border border-[#E2E8F0]">
              +{categories.length - 3}
            </span>
          )}
        </div>
...
// Loading skeleton
export function ProviderCardSkeleton({ featured = false }: { featured?: boolean }) {
  return (
    <div
      className={`
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-4 mb-4">
          <div className="flex gap-0.5">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="w-3.5 h-3.5 rounded bg-[#E2E8F0]" />
            ))}
          </div>
...
export function ProviderCardGrid({ children }: { children: React.ReactNode }) {
  return (
...
export function FeaturedProviderGrid({ children }: { children: React.ReactNode }) {
  return (
