...
export function requestStatusTone(status: string): BadgeTone {
  ...
  if (['QUALIFIED', 'ASSIGNED', 'ACCEPTED_BY_PROVIDER', 'BOOKED'].includes(s)) return 'info'
  if (['COMPLETED', 'REWARD_PENDING', 'REWARD_APPROVED', 'CLOSED', 'CONVERTED', 'PUBLISHED'].includes(s)) return 'success'
