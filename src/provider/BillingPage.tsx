...
function feeStatusBadgeClasses(status: string): string {
  switch (status) {
    case 'PAID':
...
function isDisputable(fee: any): boolean {
  if (fee.feeType !== 'QUALIFIED_LEAD') return false;
  if (fee.status !== 'PENDING' && fee.status !== 'PAID') return false;
...
export default function ProviderBillingPage() {
  ...
  const [disputeFee, setDisputeFee] = useState<any | null>(null);
  const [disputeReason, setDisputeReason] = useState<string>('WRONG_NUMBER');
  const [disputeNote, setDisputeNote] = useState('');
  const [disputeSubmitting, setDisputeSubmitting] = useState(false);
  const [disputeError, setDisputeError] = useState<string | null>(null);

  const openDisputeDialog = (fee: any) => {
    setDisputeFee(fee);
  ...
  const handleAddCard = async () => {
    ...
    try {
      ...
      setSetupError(
        err?.message || 'Could not start the payment setup. Try again.',
      );
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8 bg-[#F8FAFC] min-h-screen">
      ...
      {setupResult === 'success' && (
        <div className="rounded-[14px] bg-[#F0FDF4] border border-green-200 px-5 py-4 text-sm text-[#15803D]">
          Payment method saved. Lead fees will now be charged automatically.
        </div>
      )}
      {setupResult === 'canceled' && (
        <div className="rounded-[14px] bg-[#FEF3C7] border border-[#FDE68A] px-5 py-4 text-sm text-[#B45309]">
      ...
      <div className="bg-white p-6 rounded-[14px] border border-[#E2E8F0] shadow-sm">
        ...
        {billingLoading && (
          <div className="animate-pulse h-16 bg-[#EFF6FF] rounded-[14px]" />
        )}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            {billing.stripeConfigured ? (
              <button
                ...
                className="px-5 py-2.5 rounded-[10px] bg-[#2563EB] text-white text-sm font-semibold hover:bg-[#1D4ED8] disabled:opacity-60 transition-colors"
              >
                {redirecting
        ...
        {setupError && (
          <p className="mt-4 text-sm text-red-600">{setupError}</p>
        )}
      ...
      <div className="bg-white p-6 rounded-[14px] border border-[#E2E8F0] shadow-sm">
        ...
        <div className="space-y-4">
          ...
          {fees?.map((fee: any) => (
            <div key={fee.id} className="flex justify-between items-center gap-4 p-4 border border-[#E2E8F0] rounded-[14px] bg-[#F8FAFC]">
              <div>
              ...
              <div className="flex items-center gap-3">
                {isDisputable(fee) && (
                  <button
                    onClick={() => openDisputeDialog(fee)}
                    className="px-3 py-1.5 rounded-[10px] text-xs font-semibold border border-[#E2E8F0] text-[#475569] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors"
                  >
                    Dispute
                  </button>
                )}
      ...
      {/* Dispute dialog */}
      <Dialog open={disputeFee !== null} onOpenChange={(open) => { if (!open) setDisputeFee(null); }}>
        <DialogContent className="sm:max-w-md bg-white">
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-[10px] border border-[#E2E8F0] bg-white text-sm text-[#0F172A] focus:outline-none focus:border-[#2563EB]"
              >
            ...
            <div>
              ...
              <textarea
                ...
                maxLength={1000}
                rows={3}
                placeholder="e.g. Number was disconnected when I called within the hour."
                className="w-full px-3 py-2 rounded-[10px] border border-[#E2E8F0] bg-white text-sm text-[#0F172A] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#2563EB] resize-none"
              />
            </div>
          ...
          <DialogFooter className="gap-2 sm:gap-2">
            <button
              onClick={() => setDisputeFee(null)}
              disabled={disputeSubmitting}
              className="px-4 py-2 rounded-[10px] text-sm font-semibold border border-[#E2E8F0] text-[#475569] hover:border-[#94A3B8] disabled:opacity-60 transition-colors"
            >
              Cancel
            </button>
            <button
              ...
              disabled={disputeSubmitting}
              className="px-4 py-2 rounded-[10px] bg-[#2563EB] text-white text-sm font-semibold hover:bg-[#1D4ED8] disabled:opacity-60 transition-colors"
            >
