...
const REGULATED_TRADE_LABELS: { slug: string; label: string; hint: string }[] = [
  {
    slug: 'hvac',
 ...
 */
export function licenceFieldForCategorySlugs(slugs: string[]): { label: string; hint: string } {
  const matches = REGULATED_TRADE_LABELS.filter((t) => slugs.includes(t.slug));
  if (matches.length === 1) return { label: matches[0].label, hint: matches[0].hint };
  if (matches.length > 1) {
    return {
...
export const CREDENTIAL_REGISTRIES: { name: string; url: string; checks: string }[] = [
