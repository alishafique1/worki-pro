import { Home, Wrench, CheckCircle2, type LucideIcon } from 'lucide-react';
...
export default function StepRole({ selected, onSelect }: StepRoleProps) {
  return (
    <div>
        ...
        {options.map(({ role, Icon, title, subtitle, desc, perks, accent, bg }) => {
          ...
          return (
            <button
              ...
              className={`relative w-full text-left rounded-[20px] p-5 border-2 transition-all duration-200 ${
                isSelected
                  ? 'border-[#2563EB] shadow-[0_0_0_4px_rgba(37,99,235,0.12)]'
                  : 'border-[#E2E8F0] hover:border-[#94A3B8] hover:shadow-sm'
              }`}
              style={{ background: isSelected ? bg : '#fff' }}
            >
              <div className="flex items-start gap-4">
                <div
                  className="size-11 rounded-[14px] flex items-center justify-center shrink-0 mt-0.5"
                  style={{ background: isSelected ? accent : '#F1F5F9' }}
                >
                  <Icon
                    className="size-5"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <ul className="space-y-1">
                    {perks.map((perk) => (
                      <li key={perk} className="flex items-center gap-2 text-xs text-[#475569]">
                        <CheckCircle2
