...
export default function ProviderLayout({ children, user }: Props) {
  ...
  const businessName = (user as any)?.provider?.businessName || '';
                      ...
                      {({ isActive }) => (
                        <>
                          <span className="flex items-center gap-3">
                            <Icon
                              className={`size-4 ${isActive ? 'text-white' : 'text-white/40 group-hover:text-white/70'}`}
                              strokeWidth={2}
                            />
                            {label}
                          </span>
      ...
      <div className="relative flex flex-1 flex-col overflow-x-hidden overflow-y-auto">
        ...
        <header className="lg:hidden sticky top-0 z-10 flex w-full items-center gap-3 border-b border-[#E2E8F0] bg-white px-4 py-3 shadow-sm">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#E2E8F0] bg-white hover:bg-[#F8FAFC] transition-colors"
          >
