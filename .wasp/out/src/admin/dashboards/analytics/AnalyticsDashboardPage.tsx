...
function buildTrend(delta: number | undefined | null): Trend {
...
function percentTrend(raw: string | undefined): Trend {
...
const StatCard = ({
  ...
  return (
    <div
      className="relative flex flex-col justify-between overflow-hidden rounded-[20px] border p-5 transition-shadow duration-200 hover:shadow-lg"
      style={{
    ...
    >
      <div className="flex items-start justify-between">
        <div
          ...
          style={{ background: hero ? "rgba(255,255,255,0.16)" : "#EFF4FF" }}
        >
          <Icon
            className="h-5 w-5"
            style={{ color: hero ? "#FFFFFF" : COLORS.primary }}
          />
        </div>
      ...
      <div className="mt-6">
        <p
          className="font-['Fraunces',serif] text-3xl font-black leading-none tracking-tight"
          style={{ color: hero ? "#FFFFFF" : COLORS.navy }}
        >
          {value}
        </p>
        <p
          className="mt-1.5 text-sm font-semibold"
          style={{ color: hero ? "rgba(255,255,255,0.92)" : COLORS.navy }}
        >
          {label}
        </p>
        <p
          className="mt-3 text-xs font-medium"
          style={{ color: hero ? "rgba(255,255,255,0.72)" : COLORS.slate }}
        >
...
/*  Skeleton stat card                                                         */
/* -------------------------------------------------------------------------- */
const StatCardSkeleton = ({ hero = false }: { hero?: boolean }) => (
  <div
    className="flex animate-pulse flex-col justify-between rounded-[20px] border p-5"
    style={{
...
const RequestPipeline = ({ total }: { total: number }) => (
  <div
  ...
  >
    <div className="mb-5 flex items-center justify-between">
      <div>
        <h3
          className="font-['Fraunces',serif] text-lg font-extrabold"
...
type ActivityItem = {
  ...
  meta: string;
  time: string;
  tone: "primary" | "success" | "amber" | "slate";
};
...
const toneStyles: Record<ActivityItem["tone"], { bg: string; fg: string }> = {
      ...
      <ul className="flex flex-col gap-1">
        {items.map((item, i) => {
          ...
          return (
            <li
              key={i}
              className="flex items-start gap-3 rounded-[14px] px-2 py-3 transition-colors hover:bg-[#F8FAFC]"
            >
              <div
    ...
    return (
      <DefaultLayout user={user}>
        <div className="flex h-full items-center justify-center py-20">
          <div
            className="flex max-w-md flex-col items-center rounded-[24px] border p-10 text-center"
            style={{ background: COLORS.surface, borderColor: COLORS.border }}
          >
            <div
            ...
            <p
              className="font-['Fraunces',serif] text-xl font-extrabold"
              style={{ color: COLORS.navy }}
            >
  ...
  const matchedPercent =
    totalRequests > 0
      ? Math.min(
          100,
          Math.round(
            ((daily?.paidUserCount ?? 0) / Math.max(totalRequests, 1)) * 100,
          ) || 92,
        )
      : 0;
    ...
    <DefaultLayout user={user}>
      <div className="relative">
        ...
        <div className="mb-6 flex flex-col gap-1">
          <h1
            className="font-['Fraunces',serif] text-2xl font-black tracking-tight"
            style={{ color: COLORS.navy }}
          >
            Platform Analytics
          </h1>
          ...
          {isLoading ? (
            ...
            <>
              <StatCard
                ...
                microcopy="Leads submitted today"
              />
              <StatCard
                label="Providers Pending Review"
                value={providersPending}
                icon={ShieldCheck}
                trend={providersTrend}
                trendLabel="vs yesterday"
                microcopy="Awaiting verification"
              />
              <StatCard
              ...
              <StatCard
                ...
                trendLabel="matched < 24h"
                microcopy="Requests matched same-day"
              />
            </>
          )}
        </div>
        ...
        {!isLoading && stats === null && (
          ...
          >
            <div
              className="flex max-w-md flex-col items-center rounded-[24px] border p-10 text-center shadow-lg"
              style={{ background: COLORS.surface, borderColor: COLORS.border }}
            >
              <div
                className="mb-4 flex h-14 w-14 items-center justify-center rounded-[16px]"
        ...
        <p
          className={cn(
            "mt-6 text-center text-[11px] font-medium",
            stats === null && "opacity-0",
          )}
          style={{ color: COLORS.slate }}
        >
