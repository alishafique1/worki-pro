...
function StarRating({ rating, count }: { rating: number | null; count: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {count > 0 && (
        <span className="text-xs text-[#475569]">({count} review{count !== 1 ? "s" : ""})</span>
      )}
    </div>
  );
}
          ...
          <div className="flex flex-col md:flex-row md:items-start gap-6">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {provider.categories.map((pc) => (
                  <span
                    ...
                    className="px-3 py-1 rounded-full border border-[#BFDBFE] bg-[#EFF6FF] text-xs font-medium text-[#2563EB]"
                  >
            ...
            <div className="shrink-0">
              <Link
                ...
                className="inline-block px-6 py-3 bg-[#2563EB] text-white font-black rounded-[16px] hover:bg-[#1D4ED8] transition-colors text-sm"
              >
                Request a quote
              </Link>
            </div>
              ...
              {portfolio.map((item, i) => (
                <div key={i} className="space-y-1">
                  <img
                    ...
                    className="w-full aspect-square object-cover rounded-[14px] border border-[#E2E8F0]"
                  />
                  {item.caption && (
                    <p className="text-xs text-[#475569] text-center">{item.caption}</p>
                  )}
                </div>
              ))}
            </div>
