  ...
  return (
    <div className='flex flex-col h-full'>
      ...
      <form
        ...
        className='mt-4 flex flex-col gap-3 sm:flex-row border-t border-[#E2E8F0] pt-4'
      >
...
export default function RequestDetailPage() {
  ...
  if (isLoading) {
    return (
      <div className='p-8 max-w-4xl mx-auto min-h-[80vh] space-y-8 bg-[#F8FAFC]'>
            ...
            {provider ? (
              <div className='space-y-3'>
                <div>
                  ...
                  {provider.phone && (
                    <a href={`tel:${provider.phone}`} className='flex items-center gap-1.5 text-sm text-[#475569] hover:text-[#2563EB] transition-colors mt-1'>
                      <Phone className='size-3.5' /> {provider.phone}
                    </a>
                  )}
                  {provider.email && (
                    <a href={`mailto:${provider.email}`} className='flex items-center gap-1.5 text-sm text-[#475569] hover:text-[#2563EB] transition-colors mt-1'>
                      <Mail className='size-3.5' /> {provider.email}
                    </a>
                  )}
                </div>
                <Link
                  to={provider.slug ? `/pros/${provider.slug}` : `/pro/${provider.id}`}
