...
type SubStep = 'enter-email' | 'verify-code' | 'confirmed'
...
export default function StepVerifyEmail({ state, update, onBack, onSuccess, setError }: Props) {
  const [subStep, setSubStep] = useState<SubStep>('enter-email')
  ...
  if (subStep === 'confirmed') {
    return (
      <div className="text-center">
        ...
        <button
          onClick={() => onSuccess()}
          className="inline-flex items-center justify-center gap-2 rounded-full bg-[#2563EB] px-8 py-3.5 text-sm font-bold text-white shadow-[0_8px_24px_rgba(37,99,235,0.3)] transition-all hover:bg-[#1D4ED8]"
        >
      ...
      <div>
        ...
        <form onSubmit={handleVerify} className="space-y-6">
          ...
          <button
            ...
            className="w-full py-3 bg-[#2563EB] text-white font-bold rounded-xl hover:bg-[#1D4ED8] transition-colors disabled:opacity-50 shadow-[0_8px_24px_rgba(37,99,235,0.3)]"
          >
          ...
          <p className="text-center text-sm text-[#475569]">
            ...
            {resendCooldown > 0 ? (
              ...
              <button type="button" onClick={() => handleSendCode()} className="text-[#2563EB] font-semibold hover:underline">
                Resend code
              </button>
            )}
          </p>
        ...
        <div className="mt-6">
          <button
            type="button"
            onClick={() => { setSubStep('enter-email'); setCode(['', '', '', '', '', '']); setError(null) }}
