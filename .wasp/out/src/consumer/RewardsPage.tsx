  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-[16px] border border-[#E2E8F0] shadow p-6 space-y-4">
        {isLoading ? (
          <p className="text-[#475569]">Loading your wallet…</p>
        ) : (
          <>
      ...
      <div className="bg-white rounded-[16px] border border-[#E2E8F0] p-6 space-y-4">
        ...
        <form onSubmit={handleRedeem} className="space-y-4">
          ...
          <button
            ...
            className="w-full py-2.5 rounded-[22px] bg-[#2563EB] text-white font-bold text-sm hover:bg-[#1D4ED8] disabled:opacity-40 transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]"
          >
            {submitting
              ? 'Processing…'
              : effectivePoints >= 500
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {data.transactions.map(tx => (
              <div key={tx.id} className="flex items-center justify-between py-2.5 border-b border-[#E2E8F0] last:border-0">
