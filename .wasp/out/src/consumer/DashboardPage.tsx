    ...
    <div className="p-8 max-w-7xl mx-auto space-y-12">
      ...
      {requestsLoading && (
        <div className="grid grid-cols-3 gap-4">
          {[0, 1, 2].map((i) => (
            <div key={i} className="animate-pulse bg-white border border-[#E2E8F0] rounded-[18px] p-4 flex items-center gap-4">
          ...
          {!requestsLoading && !requestsError && requests?.length === 0 ? (
            ...
            <div className="space-y-4">
              {requests.map((req) => {
                ...
                <div
                  key={req.id}
                  className="bg-white border border-[#E2E8F0] rounded-[24px] p-6 hover:border-[#BFDBFE] hover:shadow-[0_8px_24px_rgba(37,99,235,0.08)] transition-all duration-200 group"
                >
                  <div className="flex justify-between items-start gap-4 mb-4">
                    ...
                    <span className={`shrink-0 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider ${
