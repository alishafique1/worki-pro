...
type Trend = { direction: 'up' | 'down'; label: string } | undefined;
...
function KpiCard({
  ...
  if (accent) {
    return (
      <div className="relative overflow-hidden rounded-[20px] p-6 shadow-sm bg-gradient-to-br from-[#2563EB] to-[#1D4ED8] text-white">
  ...
  return (
    <div className="bg-white rounded-[20px] border border-[#E2E8F0] p-6 space-y-1.5 shadow-sm">
...
function Bar({ value, max, color, height = 'h-2' }: { value: number; max: number; color: string; height?: string }) {
  ...
  return (
    <div className={`${height} bg-[#E2E8F0] rounded-full overflow-hidden`}>
...
export default function AnalyticsPage() {
  ...
  if (isLoading) {
    return (
      <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-8 bg-[#F8FAFC] min-h-screen">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="rounded-[20px] border border-[#E2E8F0] bg-white p-6 space-y-3">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="rounded-[20px] border border-[#E2E8F0] bg-white p-6 space-y-4">
  ...
  if (!stats) {
    return (
      <div className="p-6 md:p-8 max-w-6xl mx-auto bg-[#F8FAFC] min-h-screen">
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-8 bg-[#F8FAFC] min-h-screen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiCard
          ...
          sub={stats.pendingRequests > 0 ? 'In progress' : 'All caught up'}
          icon={Activity}
        />
        <KpiCard
          label="Points Balance"
          value={stats.currentBalance.toLocaleString()}
          sub={`${pointsToDollars(stats.currentBalance)} in rewards`}
          icon={Coins}
          accent
        />
        <KpiCard
          ...
          sub={`${stats.totalPointsRedeemed.toLocaleString()} redeemed`}
          icon={Award}
          trend={balanceTrend}
        />
      </div>
      ...
      {stats.monthlyPoints.length > 0 && (
        <div className="bg-white rounded-[24px] border border-[#E2E8F0] p-6 space-y-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            {stats.monthlyPoints.map((m) => (
              <div key={m.month} className="grid grid-cols-[64px_1fr] items-center gap-4">
                ...
                .map(([status, count]) => (
                  <div key={status} className="space-y-1.5">
                    <div className="flex items-center justify-between gap-3">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-semibold ${
                          STATUS_COLORS[status] ?? 'bg-slate-100 text-[#94A3B8]'
                        }`}
                      >
                    ...
                    <Bar
                      ...
                      color={STATUS_BAR_COLORS[status] ?? 'bg-[#2563EB]'}
                    />
                  </div>
                ))}
            </div>
      ...
      {stats.totalRequests === 0 && (
        <div className="bg-white rounded-[24px] border border-[#E2E8F0] p-12 text-center space-y-4 shadow-sm">
          <span className="flex items-center justify-center w-16 h-16 rounded-[20px] bg-[#EFF6FF] mx-auto">
          ...
          <a
            href="/request"
            className="inline-flex items-center justify-center gap-2 rounded-[14px] bg-[#2563EB] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-[#1D4ED8]"
          >
