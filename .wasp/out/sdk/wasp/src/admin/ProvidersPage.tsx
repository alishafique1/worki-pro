...
export default function AdminProvidersPage() {
  ...
  const counts = useMemo(() => {
    ...
    return {
      ALL: list.length,
      PENDING: list.filter((p: any) => p.verificationStatus === 'PENDING').length,
      VERIFIED: list.filter((p: any) => p.verificationStatus === 'VERIFIED').length,
      REJECTED: list.filter((p: any) => p.verificationStatus === 'REJECTED').length,
    };
  }, [providers]);
  ...
  const visible = useMemo(() => {
    ...
    return list.filter((p: any) => p.verificationStatus === filter);
  }, [providers, filter]);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="flex items-center gap-3">
              <h1
                className="text-4xl font-black tracking-tight text-[#0F172A]"
          ...
          {counts.PENDING > 0 && (
            <span className="inline-flex items-center gap-2 rounded-[14px] bg-[#FEF3C7] px-4 py-2.5 text-sm font-bold text-[#B45309] border border-[#F59E0B]/30">
        ...
        {/* ---- Action error ---- */}
        {actionError && (
          <div className="flex items-center justify-between rounded-[14px] border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">
            ...
            {FILTERS.map(({ key, label }) => {
              ...
              return (
                ...
                >
                  {label}
                  <span
                    className={`inline-flex min-w-[1.25rem] justify-center rounded-full px-1.5 text-xs font-bold ${
        ...
        {isLoading && (
          <div className="grid gap-5 md:grid-cols-2">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="animate-pulse rounded-[24px] border border-[#E2E8F0] bg-white p-6">
        ...
        {!isLoading && visible.length > 0 && (
          <div className="grid gap-5 md:grid-cols-2">
            {visible.map((prov: any) => (
              <ProviderCard
                key={prov.id}
                provider={prov}
                busy={busyId === prov.id}
                rejecting={rejectingId === prov.id}
                rejectReason={rejectReason}
                onApprove={() => handleApprove(prov.id)}
                onStartReject={() => {
        ...
        {/* ---- Empty state ---- */}
        {!isLoading && visible.length === 0 && (
          <div className="flex flex-col items-center justify-center rounded-[24px] border border-dashed border-[#E2E8F0] bg-white px-6 py-16 text-center">
            <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#EFF6FF]">
              {filter === 'PENDING' ? (
                <Inbox className="h-7 w-7 text-[#2563EB]" />
              ) : (
            ...
            {filter !== 'ALL' && (
              <button
                ...
                className="mt-5 inline-flex items-center rounded-[14px] bg-[#2563EB] px-4 py-2 text-sm font-bold text-white hover:bg-[#1D4ED8] transition-colors"
              >
                View all providers
              </button>
            )}
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === 'PENDING') {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-[#FEF3C7] px-2.5 py-0.5 text-xs font-bold text-[#B45309]">
  ...
  return (
    <span className="inline-flex items-center rounded-full bg-[#F1F5F9] px-2.5 py-0.5 text-xs font-bold text-[#475569]">
      {status}
    </span>
  );
}

function CredentialRow({ label, value, link }: { label: string; value?: string | null; link?: boolean }) {
      ...
      {value ? (
        isUrl ? (
          <a
            ...
            className="truncate font-semibold text-[#2563EB] hover:underline"
          >
            {value}
          </a>
        ) : (
          <span className="truncate font-semibold text-[#0F172A]">{value}</span>
        )
      ) : (
...
function ProviderCard({
  ...
  return (
    <div
      className={`flex flex-col rounded-[24px] border bg-white p-6 transition-shadow hover:shadow-md ${
        isPending
    ...
    >
      ...
      <div className="flex items-start gap-4">
        <div
          ...
          style={{ backgroundColor: colors.bg, color: colors.fg, fontFamily: 'Fraunces, serif' }}
        >
          {initial}
        </div>
        <div className="min-w-0 flex-1">
          ...
          {provider.contactName && (
            <p className="truncate text-sm text-[#475569]">{provider.contactName}</p>
          )}
        </div>
      ...
      {services.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-1.5">
          {services.slice(0, 4).map((s) => (
            <span
              key={s}
              className="inline-flex items-center rounded-full bg-[#EFF6FF] px-2.5 py-0.5 text-xs font-semibold text-[#2563EB]"
            >
              {s}
            </span>
          ))}
          ...
          {checklist.map(({ label, done }) => (
            <span
              ...
              className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold ${
                done ? 'bg-[#DCFCE7] text-[#15803D]' : 'bg-white text-[#94A3B8] border border-[#E2E8F0]'
              }`}
            >
              {done ? '✓' : '○'} {label}
            </span>
      ...
      <div className="mt-3 rounded-[14px] border border-[#E2E8F0] bg-white p-3">
        ...
        <div className="space-y-1 text-xs text-[#475569]">
          ...
          {provider.licenceUrl && (
            <CredentialRow label="Licence doc" value={provider.licenceUrl} link />
          )}
        </div>
        <div className="mt-2 border-t border-[#E2E8F0] pt-2">
          ...
          {CREDENTIAL_REGISTRIES.map((registry, i) => (
            <span key={registry.name} className="text-xs">
        ...
        {isPending && !rejecting && (
          <div className="flex gap-2">
            <button
              onClick={onApprove}
              disabled={busy}
              className="inline-flex flex-1 items-center justify-center gap-2 rounded-[14px] bg-[#22C55E] px-4 py-2.5 text-sm font-bold text-white transition-colors hover:bg-[#16A34A] disabled:opacity-60"
            >
            ...
            <button
              ...
              disabled={busy}
              className="inline-flex flex-1 items-center justify-center gap-2 rounded-[14px] border border-red-200 bg-white px-4 py-2.5 text-sm font-bold text-red-600 transition-colors hover:bg-red-50 disabled:opacity-60"
            >
        ...
        {isPending && rejecting && (
          <div className="rounded-[14px] border border-red-200 bg-red-50 p-3">
            ...
            <textarea
              ...
              rows={2}
              className="mb-2 w-full rounded-[12px] border border-red-200 bg-white p-2 text-sm text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-red-400"
            />
            <div className="flex gap-2">
              <button
                onClick={onConfirmReject}
                disabled={busy}
                className="inline-flex items-center justify-center gap-2 rounded-[14px] bg-red-600 px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-red-700 disabled:opacity-60"
              >
              ...
              <button
                ...
                disabled={busy}
                className="inline-flex items-center justify-center rounded-[14px] border border-[#E2E8F0] bg-white px-4 py-2 text-sm font-bold text-[#475569] transition-colors hover:bg-[#F8FAFC] disabled:opacity-60"
              >
                Cancel
              </button>
            </div>
