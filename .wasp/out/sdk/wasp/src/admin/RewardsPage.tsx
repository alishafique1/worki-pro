...
function consumerName(consumer: any): string {
...
function initials(consumer: any): string {
  const first = consumer?.firstName?.trim();
  const last = consumer?.lastName?.trim();
  if (first || last) {
...
function requestLabel(reward: any): string {
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="p-8 max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {!isLoading && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-[#FEF3C7] px-4 py-2 text-sm font-bold text-[#B45309]">
              ...
              <tbody>
                {rewardList.map((reward: any) => {
                  ...
                  return (
                    <tr
                      key={reward.id}
                      className={`border-b border-[#E2E8F0] last:border-0 transition-colors ${rowBg}`}
                    >
                      {/* Consumer */}
                      <td className="py-4 pl-6 pr-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col items-end gap-1.5">
                            <div className="flex gap-2">
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 rounded-[12px] bg-[#22C55E] px-3.5 py-2 text-xs font-bold text-white transition-colors hover:bg-[#16A34A] disabled:cursor-not-allowed disabled:opacity-50"
                              >
                                {isRowLoading && action === 'approve' ? (
                              ...
                              <button
                                ...
                                disabled={isRowLoading}
                                className="inline-flex items-center gap-1.5 rounded-[12px] border border-red-200 bg-white px-3.5 py-2 text-xs font-bold text-red-600 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                              >
                                {isRowLoading && action === 'reject' ? (
                                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                                ) : (
                                  <X className="h-3.5 w-3.5" />
                                )}
                            ...
                            {errorMap[reward.id] && (
                              <p className="text-xs font-medium text-red-600">{errorMap[reward.id]}</p>
                            )}
                          </div>
                        )}
                      </td>
