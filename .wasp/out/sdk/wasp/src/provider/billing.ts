...
const requireProviderForBilling = async (context: any) => {
...
export const getBillingStatus: GetBillingStatus<void, BillingStatus> = async (
  _args,
  context,
) => {
  ...
  if (!stripe || !provider.stripeCustomerId) return status;
...
export const createBillingSetupSession: CreateBillingSetupSession<
  void,
  { checkoutUrl: string }
> = async (_args, context) => {
  const provider = await requireProviderForBilling(context);
...
export async function chargeProviderFee(feeId: string): Promise<void> {
  try {
    const stripe = await getStripeOrNull();
    if (!stripe) {
      console.warn(
        `[billing] STRIPE_API_KEY not set — fee ${feeId} stays PENDING`,
      );
      return;
    }
    ...
    if (!customerId) {
      console.warn(
        `[billing] provider ${fee.providerId} has no Stripe customer — fee ${feeId} stays PENDING`,
      );
      return;
    }
    ...
    if (!paymentMethodId) {
      console.warn(
        `[billing] provider ${fee.providerId} has no card on file — fee ${feeId} stays PENDING`,
      );
      return;
    }
    ...
    console.warn(
      `[billing] charge attempt for fee ${feeId} failed — stays PENDING:`,
      err.message,
    );
  }
}
