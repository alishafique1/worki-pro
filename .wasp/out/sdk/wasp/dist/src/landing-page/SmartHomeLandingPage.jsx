...
const SMARTHOME_CHIPS = CATEGORY_QUALIFIERS['smart-home']?.detailChips ?? [];
const smartHomeFaqs = [
    {
...
export default function SmartHomeLandingPage() {
    return (<>
      <PageSeo title="Smart Home Installation in Milton, Oakville & Burlington | The Helper" description="Certified smart home installers in Milton, Oakville & Burlington. Smart locks, video doorbells, thermostats, security cameras, and full home automation. Get matched with verified pros." ogTitle="Smart Home Installation & Automation in Milton, Oakville & Burlington" ogDescription="Book certified smart home pros for security camera installation, smart lock setup, and home automation in the GTA. All major brands supported." canonicalPath="/smart-home" keywords="smart home installation Milton, smart lock Oakville, video doorbell Burlington, home automation GTA, security camera installation, smart thermostat setup" structuredData={{
      ...
      <main className="pb-32">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col items-center text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-4">
            <Link to="/get-quotes?category=smarthome&slug=smart-home" className="px-10 py-5 bg-[#2563EB] text-white font-black rounded-3xl text-lg hover:bg-[#1D4ED8] transition-all hover:-translate-y-1 shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
              Request Smart Home Quote
            </Link>
          </div>
          <p className="text-sm text-[#475569] mt-2 mb-8">Certified installers. All major brands supported. Earn rewards on every completed job.</p>

          {/* Common Problems */}
          <section className="w-full max-w-2xl mb-16" aria-label="Common smart home setups">
            <p className="text-sm font-semibold text-[#475569] uppercase tracking-widest mb-4">What would you like set up?</p>
            <div className="flex flex-wrap justify-center gap-3">
              {SMARTHOME_CHIPS.filter(chip => chip !== 'Something else').map(chip => (<Link key={chip} to={`/get-quotes?category=smarthome&slug=smart-home&problem=${encodeURIComponent(chip)}`} className="px-4 py-2 bg-white rounded-full border border-[#E2E8F0] text-sm font-medium text-[#0F172A] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors">
                  {chip}
                </Link>))}
              <Link to="/get-quotes?category=smarthome&slug=smart-home" className="px-4 py-2 bg-white rounded-full border border-[#E2E8F0] text-sm font-medium text-[#475569] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors">
                Something else →
              </Link>
            </div>
          </section>
          ...
          <section className="w-full mb-24" aria-labelledby="platforms-heading">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {['Google Home', 'Amazon Alexa', 'Apple HomeKit', 'Samsung SmartThings', 'Ring', 'Nest', 'Ecobee', 'Control4'].map((platform) => (<span key={platform} className="px-6 py-3 bg-white rounded-full border border-[#E2E8F0] text-[#0F172A] font-semibold">
                  {platform}
                </span>))}
            </div>
          </section>
          ...
          <section className="w-full max-w-2xl" aria-labelledby="faq-heading">
            ...
            <div className="rounded-2xl border border-[#E2E8F0] bg-white px-6">
              {smartHomeFaqs.map((faq) => (<FaqItem key={faq.question} question={faq.question} answer={faq.answer}/>))}
            </div>
          </section>
        </div>
      </main>
...
//# sourceMappingURL=SmartHomeLandingPage.jsx.map