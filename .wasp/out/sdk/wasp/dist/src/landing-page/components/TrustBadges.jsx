import { ShieldCheck, Bolt, HardHat, BadgeCheck, Search, ClipboardCheck, UserCheck, } from "lucide-react";
...
export default function TrustBadges() {
    return (<section className="bg-[#F8FAFC] py-16 sm:py-20">
      <Container>
        <SectionHeader eyebrow="LICENSED. INSURED. VERIFIED." title="Every pro is verified before they can accept a job." description="Real licensing bodies, real insurance, real reviews — not marketing claims."/>

        {/* ── Badge row ─────────────────────────────────────────────── */}
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {trustBadges.map((badge) => (<article key={badge.title} className="flex h-full flex-col rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-[0_4px_18px_rgba(15,23,42,0.04)] transition duration-200 hover:border-[#BFDBFE] hover:shadow-[0_8px_28px_rgba(15,23,42,0.06)]">
              ...
              <a href={badge.href} target={badge.href.startsWith("http") ? "_blank" : undefined} rel={badge.href.startsWith("http") ? "noopener noreferrer" : undefined} className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-[#2563EB] hover:underline">
                {badge.hrefLabel}
        ...
        <div className="mt-14">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {verifySteps.map((step, idx) => (<div key={step.title} className="flex h-full gap-4 rounded-2xl border border-[#E2E8F0] bg-white p-5">
...
//# sourceMappingURL=TrustBadges.jsx.map