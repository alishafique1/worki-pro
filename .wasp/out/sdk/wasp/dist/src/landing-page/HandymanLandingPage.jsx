...
const HANDYMAN_CHIPS = CATEGORY_QUALIFIERS['handyman']?.detailChips ?? [];
const handymanFaqs = [
    {
...
export default function HandymanLandingPage() {
    return (<>
      <PageSeo title="Handyman Services in Oakville, Burlington & Milton | The Helper" description="Trusted handyman pros for TV mounting, furniture assembly, drywall repair, and home repairs in Oakville, Burlington & Milton. Background-checked, insured, and verified." ogTitle="Handyman Services in Oakville, Burlington & Milton | The Helper" ogDescription="Book background-checked handyman pros for TV mounting, furniture assembly, painting, and home repairs in the GTA. Verified and insured." canonicalPath="/handyman" keywords="handyman Oakville, handyman services Burlington, TV mounting Milton, furniture assembly GTA, drywall repair, home repairs near me" structuredData={{
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <LandingHeroBanner src="https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=1600&q=80" alt="Handyman completing home repairs in Milton"/>
      <main className="pb-32">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col items-center text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-4">
            <Link to="/get-quotes?category=handyman&slug=handyman" className="px-10 py-5 bg-[#2563EB] text-white font-black rounded-3xl text-lg hover:bg-[#1D4ED8] transition-all hover:-translate-y-1 shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
              Get Handyman Help
            </Link>
          </div>
          <p className="text-sm text-[#475569] mt-2 mb-8">Background-checked pros. Earn rewards on every completed job.</p>

          {/* Common Problems */}
          <section className="w-full max-w-2xl mb-16" aria-label="Common handyman tasks">
            <p className="text-sm font-semibold text-[#475569] uppercase tracking-widest mb-4">What needs to be done?</p>
            <div className="flex flex-wrap justify-center gap-3">
              {HANDYMAN_CHIPS.filter(chip => chip !== 'Something else').map(chip => (<Link key={chip} to={`/get-quotes?category=handyman&slug=handyman&problem=${encodeURIComponent(chip)}`} className="px-4 py-2 bg-white rounded-full border border-[#E2E8F0] text-sm font-medium text-[#0F172A] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors">
                  {chip}
                </Link>))}
              <Link to="/get-quotes?category=handyman&slug=handyman" className="px-4 py-2 bg-white rounded-full border border-[#E2E8F0] text-sm font-medium text-[#475569] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors">
                Something else →
              </Link>
            </div>
          </section>
          ...
          <section className="w-full mb-24" aria-labelledby="service-areas-heading">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {['Oakville', 'Burlington', 'Milton', 'Mississauga', 'Brampton', 'Halton Hills'].map((area) => (<Link key={area} to={`/areas/${area.toLowerCase().replace(' ', '-')}`} className="px-6 py-3 bg-white rounded-full border border-[#E2E8F0] text-[#0F172A] font-semibold hover:border-[#2563EB] hover:text-[#2563EB] transition-colors">
                  Handyman in {area}
                </Link>))}
            </div>
          </section>
          ...
          <section className="w-full max-w-2xl" aria-labelledby="faq-heading">
            ...
            <div className="rounded-2xl border border-[#E2E8F0] bg-white px-6">
              {handymanFaqs.map((faq) => (<FaqItem key={faq.question} question={faq.question} answer={faq.answer}/>))}
            </div>
          </section>
        </div>
      </main>
...
//# sourceMappingURL=HandymanLandingPage.jsx.map