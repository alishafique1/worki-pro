...
import { AlertTriangle, ChevronDown, ChevronUp, DollarSign, Wrench } from 'lucide-react';
...
export default function PlumbingLandingPage() {
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <LandingHeroBanner src="https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&w=1600&q=80" alt="Licensed plumber fixing pipes in an Oakville home"/>
      <main className="pb-32">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col items-center text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-4">
            <Link to="/get-quotes?category=plumbing&slug=plumbing" className="px-10 py-5 bg-[#2563EB] text-white font-black rounded-3xl text-lg hover:bg-[#1D4ED8] transition-all hover:-translate-y-1 shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
          ...
          <section className="w-full max-w-2xl mb-16" aria-label="Common plumbing problems">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {PLUMBING_CHIPS.filter(chip => chip !== 'Something else').map(chip => (<Link key={chip} to={`/get-quotes?category=plumbing&slug=plumbing&problem=${encodeURIComponent(chip)}`} className="px-4 py-2 bg-white rounded-full border border-[#E2E8F0] text-sm font-medium text-[#0F172A] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors">
                  {chip}
                </Link>))}
              <Link to="/get-quotes?category=plumbing&slug=plumbing" className="px-4 py-2 bg-white rounded-full border border-[#E2E8F0] text-sm font-medium text-[#475569] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors">
                Something else →
              </Link>
            </div>
          </section>
...
//# sourceMappingURL=PlumbingLandingPage.jsx.map