...
export default function HvacLandingPage() {
    return (<>
      <PageSeo title="HVAC Repair & Installation Milton, Oakville, Burlington | The Helper" description="Licensed TSSA HVAC contractors for furnace repair, AC installation, and air quality services in Milton, Oakville & Burlington. Same-day emergency service available. Get matched with verified pros." ogTitle="HVAC Services in Milton, Oakville & Burlington | The Helper" ogDescription="Find trusted HVAC pros for furnace repair, AC tune-ups, and heating installation in the GTA. Licensed, insured, and verified contractors." canonicalPath="/hvac" keywords="HVAC repair Milton, furnace repair Oakville, AC installation Burlington, heating services GTA, air conditioning repair, HVAC contractor near me" structuredData={{
      ...
      <main className="pb-32">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col items-center text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-4">
            <Link to="/get-quotes?category=hvac&slug=hvac" className="px-10 py-5 bg-blue-600 text-white font-black rounded-3xl text-lg hover:bg-blue-700 transition-all hover:-translate-y-1 shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
              Request HVAC Quote
            </Link>
          </div>
          ...
          <section className="w-full max-w-2xl mb-16" aria-label="Common HVAC problems">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {HVAC_CHIPS.filter(chip => chip !== 'Something else').map(chip => (<Link key={chip} to={`/get-quotes?category=hvac&slug=hvac&problem=${encodeURIComponent(chip)}`} className="px-4 py-2 bg-white rounded-full border border-[#E2E8F0] text-sm font-medium text-[#0F172A] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors">
                  {chip}
                </Link>))}
              <Link to="/get-quotes?category=hvac&slug=hvac" className="px-4 py-2 bg-white rounded-full border border-[#E2E8F0] text-sm font-medium text-[#475569] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors">
                Something else →
              </Link>
            </div>
          </section>
...
//# sourceMappingURL=HvacLandingPage.jsx.map