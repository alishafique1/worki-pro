...
export default function LandingPage() {
    return (<>
      <PageSeo title="Book a Verified Home Pro | The Helper" description="Stop calling around. Submit one request and get matched with a licensed, insured HVAC, plumbing, electrical, or handyman pro in Milton, Oakville & Burlington. 100% free. Earn $60+ cash back." ogTitle="Book a Verified Home Pro | The Helper" ogDescription="One request. Matched with a verified local pro within 24 hours. Same-day service available. 100% free for homeowners. Earn $60+ cash back on your first job." canonicalPath="/" keywords="home services GTA, find HVAC pro Milton, plumber Oakville, electrician Burlington, handyman GTA, home repair marketplace, verified contractors Toronto" structuredData={createLocalBusinessSchema({
      ...
      <main className="min-h-screen bg-white font-sans">
          ...
          <Container className="relative grid items-center gap-8 md:gap-12 md:grid-cols-2 lg:gap-16">
            ...
            <div>
              ...
              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                <Link to="/get-quotes" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#2563EB] px-7 py-3.5 text-base font-semibold text-white shadow-[0_8px_28px_rgba(37,99,235,0.35)] transition duration-200 hover:bg-[#1D4ED8] hover:shadow-[0_12px_36px_rgba(37,99,235,0.45)]">
                  Get matched now
              ...
              <div className="mt-7 grid grid-cols-2 gap-x-4 gap-y-2.5 text-sm text-[#475569]">
                {["100% Free for Homeowners", "Licensed & Insured Pros", "Same-Day Service", "Verified Reviews Only"].map((item) => (<span key={item} className="flex items-center gap-2">
            ...
            <div className="relative">
              <div className="relative overflow-hidden rounded-3xl shadow-[0_32px_80px_rgba(15,23,42,0.18)]">
                <img src={heroImage} alt="Verified professional completing a home service" className="h-72 w-full object-cover sm:h-80"/>
        ...
        <section id="how-it-works" className="bg-white py-16 sm:py-20">
          <Container>
            <SectionHeader eyebrow="HOW IT WORKS" title="Submit once. Matched within 24 hours." description="Tell us what you need. We handle the matching, scheduling, and follow-up."/>
            ...
            <div className="mt-10 text-center">
              <Link to="/get-quotes" className="inline-flex items-center gap-2 rounded-full bg-[#2563EB] px-7 py-3.5 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(37,99,235,0.3)] transition duration-200 hover:bg-[#1D4ED8]">
          ...
          <Container>
            <div className="grid gap-10 lg:grid-cols-2 lg:items-start">
              ...
              <div className="flex flex-col gap-3">
                {rewardTiers.map((tier) => (<div key={tier.label} className="flex items-center gap-4 rounded-xl border border-white/15 bg-white/10 px-5 py-4 backdrop-blur-sm transition hover:bg-white/15">
        ...
        <section className="bg-[#0F172A] py-16 sm:py-20">
          <Container>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {trustPillars.map((pillar) => (<article key={pillar.title} className="rounded-2xl border border-white/8 bg-white/5 p-6 transition duration-200 hover:bg-white/8">
        ...
        <section className="bg-[#F8FAFC] py-16 sm:py-20">
          <Container>
            ...
            <div className="mt-10 grid gap-5 sm:grid-cols-3">
              {testimonials.map((t) => (<article key={t.name} className="relative overflow-hidden rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-[0_8px_32px_rgba(15,23,42,0.05)]">
                  <div className="absolute right-5 top-4 select-none font-serif text-[80px] leading-none text-[#2563EB]/8">"</div>
                  <div className="mb-4 flex gap-0.5">
                    {[...Array(5)].map((_, i) => (<span key={i} className="text-[#2563EB] text-sm">★</span>))}
                  </div>
        ...
        ].map(({ value, label }) => (<div key={label} className="text-center">
        ...
        <section className="bg-white py-12 sm:py-16 border-t border-[#E2E8F0]">
          <Container>
            ...
            { icon: <BadgeCheck className="size-3.5"/>, text: "Free to apply" },
              ...
              <Link to="/providers/apply" className="inline-flex shrink-0 items-center gap-2 rounded-full bg-[#0F172A] px-7 py-3.5 text-sm font-semibold text-white shadow transition duration-150 hover:bg-[#1E293B]">
