...
export default function ServiceAreaLandingPage() {
    ...
    return (<div className="min-h-screen bg-[#F8FAFC]">
      <PageSeo title={`${serviceLabel} in ${areaName} | Get Matched Today`} description={`Find vetted ${serviceLabel.toLowerCase()} pros in ${areaName}. One request — get matched fast, book quickly, earn reward points on your job.`} ogTitle={`${serviceLabel} in ${areaName} | The Helper`} ogDescription={`Book verified ${serviceLabel.toLowerCase()} pros in ${areaName}. Matched fast, vetted, and backed by The Helper rewards.`} canonicalPath={`/services/${serviceSlug}/${areaSlug}`} keywords={`${serviceLabel.toLowerCase()} ${areaName}, ${serviceLabel.toLowerCase()} ${areaSlug}, ${serviceLabel.toLowerCase()} GTA, verified ${serviceLabel.toLowerCase()} pro`} structuredData={jsonLd}/>
      ...
      <section className="max-w-5xl mx-auto px-4 pb-16">
        ...
        {!isLoading && providers && providers.length > 0 && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {providers.map((pro) => (<Link key={pro.id} to={pro.slug ? `/pros/${pro.slug}` : `/pro/${pro.id}`} className="block rounded-[18px] border border-[#E2E8F0] bg-white p-5 hover:border-[#BFDBFE] transition-all group">
