...
export default function AccountPage() {
    ...
    return (<div className="min-h-screen bg-[#F8FAFC] py-10 px-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" className={inputClass} placeholder="Jane" value={firstName} onChange={e => setFirstName(e.target.value)} required/>
              </div>
              <div>
            ...
            <div>
              ...
              <input type="tel" className={inputClass} placeholder="(416) 555-0100" value={phone} onChange={e => setPhone(e.target.value)} required/>
            </div>

            {/* Postal Code */}
            <div>
              <label className={labelClass}>Postal Code *</label>
              <input type="text" className={`${inputClass} uppercase`} placeholder="L9T 2X5" value={postalCode} onChange={e => setPostalCode(e.target.value.toUpperCase())} maxLength={7} required/>
            ...
            {/* Feedback messages */}
            {success && (<div className="flex items-center gap-2 bg-[#F0FDF4] border border-[#22C55E] rounded-xl px-4 py-3 text-sm text-[#15803D]">
                <svg className="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/>
                </svg>
                Profile saved
              </div>)}
            {error && (<div className="text-red-600 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm">
                {error}
              </div>)}

            {/* Save button */}
            <div className="pt-2">
              <button type="submit" disabled={saving} className="bg-[#2563EB] text-white font-semibold rounded-[14px] px-6 py-3 text-sm hover:bg-[#1D4ED8] transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
