    ...
    async function handleNext() {
        ...
        try {
            await completeOnboardingFn({
                ...
                serviceAreas: form.role === 'PROVIDER' && form.serviceAreas.trim()
    ...
    return (<div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-6">
      <div className="w-full max-w-lg">
        ...
        <div className="h-1.5 bg-[#E2E8F0] rounded-full mb-8 overflow-hidden">
          <div className="h-full bg-[#2563EB] transition-all duration-700 ease-out rounded-full" style={{ width: `${progress}%` }}/>
        </div>
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-[24px] p-8 shadow-lg">
          ...
          {step === 1 && (<div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                { role: 'CONSUMER', icon: '🏠', title: 'Homeowner', desc: 'I need home services' },
                { role: 'PROVIDER', icon: '🔧', title: 'Service Pro', desc: 'I provide home services' },
            ].map(option => (<button key={option.role} type="button" onClick={() => updateForm('role', option.role)} className={`border-2 rounded-[24px] p-8 cursor-pointer transition-all text-left ${form.role === option.role
          ...
          {step === 2 && (<div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-4">
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" className={inputClass} placeholder="Jane" value={form.firstName} onChange={e => updateForm('firstName', e.target.value)}/>
                </div>
                <div>
                  <label className={labelClass}>Last Name</label>
                  <input type="text" className={inputClass} placeholder="Smith" value={form.lastName} onChange={e => updateForm('lastName', e.target.value)}/>
                </div>
              </div>
              <div>
                <label className={labelClass}>Phone *</label>
                <input type="tel" className={inputClass} placeholder="(416) 555-0100" value={form.phone} onChange={e => updateForm('phone', e.target.value)}/>
              </div>
              <div>
                <label className={labelClass}>Postal Code *</label>
                <input type="text" className={inputClass} placeholder="M5V 3A8" value={form.postalCode} onChange={e => updateForm('postalCode', e.target.value)}/>
              </div>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" className="mt-0.5 accent-[#2563EB]" checked={form.smsConsent} onChange={e => updateForm('smsConsent', e.target.checked)}/>
                <span className="text-sm text-[#475569]">
                  I agree to receive SMS updates about my service requests
                </span>
              </label>
              {form.role === 'CONSUMER' && (<div>
                  <label className={labelClass}>Referral Code <span className="font-normal opacity-60">(optional)</span></label>
                  <input type="text" className={inputClass} placeholder="REF-XXXXXX" value={form.referralCode} onChange={e => updateForm('referralCode', e.target.value.toUpperCase())}/>
          ...
          {step === 3 && form.role === 'PROVIDER' && (<div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-4">
              <div>
              ...
              <div>
                ...
                <input type="text" className={inputClass} placeholder="Smith HVAC Services" value={form.businessName} onChange={e => updateForm('businessName', e.target.value)}/>
              </div>
              <div>
                <label className={labelClass}>Service Areas</label>
                <input type="text" className={inputClass} placeholder="e.g. M5V, M6G, Toronto" value={form.serviceAreas} onChange={e => updateForm('serviceAreas', e.target.value)}/>
          ...
          <div className="flex items-center justify-between mt-8">
            {step > 1 ? (<button type="button" onClick={handleBack} className="px-6 py-3 font-bold text-[#475569] hover:text-[#0F172A] transition-colors">
                ← Back
              </button>) : (<div />)}
