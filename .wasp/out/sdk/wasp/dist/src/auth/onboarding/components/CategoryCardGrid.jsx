...
function Skeleton() {
    return (<div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      {Array.from({ length: 6 }).map((_, i) => (<div key={i} className="rounded-2xl border border-[#E2E8F0] p-5 animate-pulse">
...
export default function CategoryCardGrid({ title, subtitle, selectedIds, onToggle, max }) {
    ...
    if (isError) {
        ...
        <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-center">
          ...
          <p className="text-xs text-red-400 mt-2 opacity-60">{error?.message || 'Connection error'}</p>
        </div>
      </div>);
    }
    ...
    return (<div>
      <div className="flex items-center justify-between mb-1">
        ...
        {selectedIds.length > 0 && (<span className="text-xs font-semibold text-[#2563EB] bg-[#EFF6FF] rounded-full px-3 py-1">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {parents.map((cat) => {
            ...
            return (<button key={cat.id} type="button" disabled={atLimit} onClick={() => onToggle(cat.id)} className={`group relative rounded-2xl border-2 text-left transition-all cursor-pointer overflow-hidden ${selected
                    ? 'border-[#2563EB] bg-[#EFF6FF]'
                    : atLimit
                        ? 'border-[#E2E8F0] bg-[#F8FAFC] opacity-40 cursor-not-allowed'
                        : 'border-[#E2E8F0] bg-white hover:border-[#94A3B8] hover:shadow-sm'}`}>
              {cat.imageUrl && (<div className="relative h-20 sm:h-24 overflow-hidden">
                  <img src={cat.imageUrl} alt="" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" loading="lazy"/>
                  <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent"/>
                </div>)}
              <div className={`p-5 ${cat.imageUrl ? 'pt-3' : ''}`}>
                <div className={`size-10 rounded-xl flex items-center justify-center mb-3 ${selected ? 'bg-[#2563EB] text-white' : 'bg-[#EFF6FF] text-[#2563EB]'}`}>
                  <Icon className="size-5"/>
                </div>
                <p className={`text-sm font-bold ${selected ? 'text-[#2563EB]' : 'text-[#0F172A]'}`}>{cat.name}</p>
...
//# sourceMappingURL=CategoryCardGrid.jsx.map