...
import { MapPin, Zap, Award, Search, ArrowLeft, BadgeCheck, ImageOff, ExternalLink, Star, ChevronLeft, ChevronRight, } from "lucide-react";
...
function StarRating({ rating, count }) {
    ...
    return (<div className="flex items-center gap-2">
      ...
      {count > 0 && (<span className="text-xs text-[#475569]">
...
function PortfolioImage({ url, caption, index, onClick, }) {
    const [failed, setFailed] = useState(false);
    if (failed) {
        return (<div className="w-full aspect-square rounded-[14px] border border-[#E2E8F0] bg-[#F1F5F9] flex flex-col items-center justify-center gap-2 cursor-pointer" onClick={onClick}>
    ...
    return (<button onClick={onClick} className="block w-full text-left space-y-1">
      <div className="relative overflow-hidden rounded-[14px] border border-[#E2E8F0] aspect-square bg-[#F1F5F9]">
        <img src={url} alt={caption ?? `Portfolio item ${index + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" onError={() => setFailed(true)} loading="lazy"/>
        <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-colors"/>
      </div>
      {caption && (<p className="text-xs text-[#475569] text-center px-1">{caption}</p>)}
    </button>);
}
// Lightbox for portfolio
function PortfolioLightbox({ items, startIndex, onClose, }) {
    ...
    return (<div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={onClose}>
      <div className="relative max-w-4xl w-full max-h-[90vh] flex flex-col items-center gap-3" onClick={(e) => e.stopPropagation()}>
        {/* Close */}
        <button onClick={onClose} className="absolute -top-10 right-0 text-white/70 hover:text-white text-sm">
          Close ✕
        </button>

        {/* Image */}
        <div className="relative w-full flex items-center justify-center bg-black rounded-2xl overflow-hidden max-h-[75vh]">
          {failed ? (<div className="flex flex-col items-center gap-3 py-20 text-white/40">
              <ImageOff className="size-8"/>
              <span className="text-sm">Image unavailable</span>
            </div>) : (<img src={item.url} alt={item.caption ?? `Portfolio item ${idx + 1}`} className="max-h-[75vh] max-w-full object-contain" onError={() => setFailed(true)}/>)}

          {items.length > 1 && (<>
              <button onClick={() => setIdx((p) => (p - 1 + items.length) % items.length)} className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 hover:bg-black/80 flex items-center justify-center text-white transition-colors">
                <ChevronLeft className="size-5"/>
              </button>
              <button onClick={() => setIdx((p) => (p + 1) % items.length)} className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 hover:bg-black/80 flex items-center justify-center text-white transition-colors">
...
export default function ProPublicPage() {
    ...
    const { data: similarRaw } = useQuery(getProviders, { categorySlug: firstCategorySlug }, { enabled: !!firstCategorySlug });
    ...
    const accreditations = provider.accreditationsJson
        ? JSON.parse(provider.accreditationsJson)
        : [];
    ...
    return (<div className="min-h-screen bg-[#F8FAFC]">
      ...
      {/* Portfolio lightbox */}
      {lightboxIdx !== null && portfolio.length > 0 && (<PortfolioLightbox items={portfolio} startIndex={lightboxIdx} onClose={() => setLightboxIdx(null)}/>)}
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="rounded-[24px] border border-[#E2E8F0] bg-white p-8">
          <div className="flex flex-col md:flex-row md:items-start gap-6">
            ...
            <div className="shrink-0">
              <ProfilePhoto url={provider.profilePhotoUrl} name={provider.businessName}/>
            </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-3 mb-1">
                ...
                {provider.verificationStatus === "VERIFIED" && (<span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-green-50 text-[#22C55E] border border-green-200">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {provider.categories.map((pc) => (<Link key={pc.serviceCategoryId} to={`/services/${pc.serviceCategory.slug}`} className="px-3 py-1 rounded-full border border-[#BFDBFE] bg-[#EFF6FF] text-xs font-medium text-[#2563EB] hover:bg-[#DBEAFE] transition-colors">
              ...
              <div className="flex flex-wrap gap-4 text-sm text-[#475569]">
                {provider.serviceAreas?.length > 0 && (<span className="inline-flex items-center gap-1.5">
                    <MapPin className="size-4"/>
                    {provider.serviceAreas.join(", ")}
                  </span>)}
                {provider.responseTimeMins && (<span className="inline-flex items-center gap-1.5">
                ...
                {provider.website && (<a href={provider.website.startsWith("http") ? provider.website : `https://${provider.website}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 hover:underline text-[#2563EB]">
            ...
            {/* CTA */}
            <div className="shrink-0">
              <Link to={`/get-quotes?provider=${provider.id}`} className="inline-block px-6 py-3 bg-[#2563EB] text-white font-black rounded-[16px] shadow-[0_8px_24px_rgba(37,99,235,0.3)] hover:bg-[#1D4ED8] transition-colors text-sm whitespace-nowrap">
                Request a quote
              </Link>
            </div>
          </div>
        ...
        {portfolio.length > 0 && (<div className="rounded-[24px] border border-[#E2E8F0] bg-white p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link to={`/get-quotes?provider=${provider.id}`} className="text-sm font-bold text-[#2563EB] hover:underline whitespace-nowrap">
                Request a quote →
              </Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {portfolio.map((item, i) => (<PortfolioImage key={i} url={item.url} caption={item.caption} index={i} onClick={() => setLightboxIdx(i)}/>))}
            </div>
          </div>)}
        ...
        {/* Similar providers */}
        {similarProviders.length > 0 && (<div className="rounded-[24px] border border-[#E2E8F0] bg-white p-8">
            <div className="flex items-center justify-between mb-6">
              <div>
              ...
              <Link to={`/discover?category=${firstCategorySlug}`} className="text-sm font-bold text-[#2563EB] hover:underline whitespace-nowrap">
                See all →
              </Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {similarProviders.map((p) => {
                ...
                const initials = p.businessName
                    ...
                    .slice(0, 2)
                    .toUpperCase();
                return (<Link key={p.slug} to={`/pros/${p.slug}`} className="flex items-center gap-4 p-4 rounded-[18px] border border-[#E2E8F0] hover:border-[#BFDBFE] hover:shadow-sm transition-all">
                    <SimilarProviderAvatar url={p.profilePhotoUrl} initials={initials}/>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-sm text-[#0F172A] truncate">
                        {p.businessName}
                      </p>
                      {catName && (<p className="text-xs text-[#475569] truncate">{catName}</p>)}
        ...
        <div className="rounded-[24px] bg-[#0F172A] p-8 text-center">
          ...
          <Link to={`/get-quotes?provider=${provider.id}`} className="inline-block px-8 py-4 bg-[#2563EB] text-white font-black rounded-[18px] hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
            Request a quote
          </Link>
        </div>
      </div>
...
function ProfilePhoto({ url, name }) {
    ...
    if (!url || failed) {
        return (<div className="w-24 h-24 rounded-[18px] bg-[#EFF6FF] border border-[#BFDBFE] flex items-center justify-center">
        <span className="text-4xl font-black text-[#2563EB]">{initial}</span>
      </div>);
    }
    return (<img src={url} alt={name} className="w-24 h-24 rounded-[18px] object-cover border border-[#E2E8F0]" onError={() => setFailed(true)}/>);
}
function SimilarProviderAvatar({ url, initials, }) {
    const [failed, setFailed] = useState(false);
    if (!url || failed) {
        return (<div className="w-12 h-12 rounded-[12px] bg-[#EFF6FF] border border-[#BFDBFE] flex items-center justify-center shrink-0">
        <span className="text-sm font-black text-[#2563EB]">{initials}</span>
      </div>);
    }
    return (<img src={url} alt="" className="w-12 h-12 rounded-[12px] object-cover border border-[#E2E8F0] shrink-0" onError={() => setFailed(true)}/>);
}
