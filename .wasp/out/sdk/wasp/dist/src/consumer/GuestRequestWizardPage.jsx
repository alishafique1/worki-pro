...
export default function GuestRequestWizardPage() {
    ...
    const knownCat = HARDCODED_CATEGORIES.find(c => c.slug === slugParam || c.slug === categoryParam || c.id === categoryParam) ?? null;
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-[24px] p-8 shadow-lg">
          <div className="animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            {step === 3 && (<StepDetails state={state} update={update} onNext={handleDetailsNext} onBack={back} prefilled={isLoggedIn}/>)}
            {/* Step 4 only shown for guests — logged-in users have totalSteps=3 */}
            {step === 4 && (<StepOtp state={state} onBack={back} onVerified={handleSubmit} externalError={submitError} email={state.email}/>)}
          </div>
          ...
          {isLoggedIn && submitError && step === 3 && (<p className="mt-4 text-sm text-red-500 bg-red-500/10 border border-red-500/20 rounded-[10px] px-4 py-3">
              {submitError}
            </p>)}
        </div>
      </div>
