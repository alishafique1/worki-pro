...
function RequestCard({ req }) {
    ...
    return (<article className="space-y-5 rounded-[24px] border border-[#E2E8F0] bg-white p-5 shadow-sm">
      {/* Floating detail link */}
      <div className="flex justify-end">
        <Link to={`/my-requests/${req.id}`} className="text-xs font-bold text-[#2563EB] hover:underline">
          Open details →
        </Link>
      </div>
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
        <div className="min-w-0">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${statusColor(req.status)}`}>
        ...
        <div className="rounded-[18px] border border-[#E2E8F0] bg-[#F8FAFC] p-4 md:min-w-[260px]">
          ...
          {appointment?.scheduledAt && ['CONFIRMED', 'BOOKED', 'RESCHEDULED'].includes(appointment.status) && (<div className="mt-3">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-[18px] border border-[#E2E8F0] bg-[#F8FAFC] p-4">
          ...
          <div className="space-y-1 text-sm text-[#475569]">
            ...
            {req.preferredTime && (<p>Best contact time: {req.preferredTime.replace(/_/g, " ")}</p>)}
          </div>
        </div>
      </div>
...
export default function MyRequestsPage() {
    ...
    return (<div className="mx-auto max-w-5xl space-y-6 p-6 bg-[#F8FAFC] min-h-screen">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <Link to="/request-service" className="inline-flex items-center justify-center gap-2 rounded-[22px] bg-[#2563EB] px-5 py-3 text-sm font-bold text-white hover:bg-[#1D4ED8] transition-colors">
          <Wrench className="size-4"/> Request Service
        </Link>
      </div>

      {isLoading && (<div className="space-y-4">
          {[0, 1].map((item) => (<div key={item} className="rounded-[24px] border border-[#E2E8F0] bg-white p-5">
      ...
      {!isLoading && !error && !requests?.length && (<div className="flex flex-col items-center space-y-3 rounded-[24px] border border-[#E2E8F0] bg-white p-12 text-center">
          ...
          <Link to="/request-service" className="mt-2 rounded-[22px] bg-[#2563EB] px-6 py-2.5 text-sm font-bold text-white hover:bg-[#1D4ED8] transition-colors">
            Request a Service
          </Link>
        </div>)}

      {!error && !!requests?.length && (<div className="space-y-5">
