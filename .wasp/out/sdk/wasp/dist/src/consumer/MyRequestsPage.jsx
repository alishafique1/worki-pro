...
import { CalendarClock, CheckCircle2, Clock3, Inbox, MapPin, MessageSquareText, Send, ShieldCheck, UserRound, Wrench, } from "lucide-react";
...
function MessageComposer({ requestId }) {
    ...
    return (<div className="mt-4 space-y-2">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:flex-row">
        ...
        <input id={`message-${requestId}`} value={body} onChange={(event) => setBody(event.target.value)} placeholder="Ask a question or share an update..." maxLength={1000} className="min-w-0 flex-1 rounded-[12px] border border-[#E2E8F0] bg-white px-4 py-3 text-sm outline-none focus:border-[#2563EB] focus:ring-1 focus:ring-[#2563EB]/30"/>
        <button type="submit" disabled={isSending || !body.trim()} className="inline-flex items-center justify-center gap-2 rounded-[14px] bg-[#2563EB] px-5 py-3 text-sm font-bold text-white hover:bg-[#1D4ED8] transition-colors disabled:cursor-not-allowed disabled:opacity-50 shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
...
function MetaItem({ icon, children, }) {
    return (<span className="inline-flex items-center gap-1.5 text-xs font-medium text-[#475569]">
...
function RequestCard({ req }) {
    ...
    return (<article className="overflow-hidden rounded-[24px] border border-[#E2E8F0] bg-white shadow-sm transition-shadow hover:shadow-md" style={{ borderLeft: `4px solid ${accentColor(req.status)}` }}>
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2">
              ...
              {(req.city || req.postalCode) && (<MetaItem icon={<MapPin className="size-3.5"/>}>
                  {[req.city, req.postalCode].filter(Boolean).join(", ")}
                </MetaItem>)}
              {provider ? (<span className="inline-flex items-center gap-1.5 text-xs font-medium text-[#475569]">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2">
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${urgencyStyle(req.urgency)}`}>
            {req.urgency.charAt(0) + req.urgency.slice(1).toLowerCase()}
          </span>
          <Link to={`/account/requests/${req.id}`} className="text-xs font-bold text-[#2563EB] hover:underline">
            Open details →
          </Link>
          {!appointment?.scheduledAt &&
            ["ASSIGNED", "ACCEPTED_BY_PROVIDER", "QUALIFIED"].includes(req.status) && (<a href={`/account/book/${req.id}`} className="text-xs font-bold text-[#2563EB] hover:underline">
                Book appointment →
              </a>)}
          <button type="button" onClick={() => setExpanded((v) => !v)} className="ml-auto text-xs font-bold text-[#475569] hover:text-[#0F172A]">
            {expanded
            ? "Hide"
            : `Track & message${messages.length ? ` (${messages.length})` : ""}`}
          </button>
        </div>
      </div>

      {/* Expandable body */}
      {expanded && (<div className="space-y-5 border-t border-[#E2E8F0] bg-[#F8FAFC]/40 p-5">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-[20px] border border-[#E2E8F0] bg-white p-4">
              ...
              {appointment?.status && (<p className="mt-1 text-xs text-[#94A3B8]">
                  Status: {formatStatus(appointment.status)}
                </p>)}
              {appointment?.scheduledAt &&
                ["CONFIRMED", "BOOKED", "RESCHEDULED"].includes(appointment.status) && (<div className="mt-3">
                    ...
                    description: `${req.description}${provider
                        ...
                        : ""}`,
                    startTime: new Date(appointment.scheduledAt),
                    endTime: new Date(new Date(appointment.scheduledAt).getTime() +
                        60 * 60 * 1000),
                    location: [req.city, req.postalCode]
                        .filter(Boolean)
                        .join(", ") || undefined,
                }}/>
                  </div>)}
              {!appointment?.scheduledAt &&
                ["ASSIGNED", "ACCEPTED_BY_PROVIDER", "QUALIFIED"].includes(req.status) && (<a href={`/account/book/${req.id}`} className="mt-3 inline-flex items-center gap-1.5 rounded-[14px] bg-[#2563EB] px-3 py-2 text-xs font-bold text-white hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
                    Book Appointment
                  </a>)}
            </div>
            ...
            <div className="rounded-[20px] border border-[#E2E8F0] bg-white p-4">
              ...
              {provider ? (<div className="space-y-1 text-sm text-[#475569]">
            ...
            <div className="rounded-[20px] border border-[#E2E8F0] bg-white p-4">
              ...
              <div className="space-y-1 text-sm text-[#475569]">
                ...
                {(req.city || req.postalCode) && (<p>{[req.city, req.postalCode].filter(Boolean).join(", ")}</p>)}
                {req.estimatedSchedule && (<p>Timeline: {req.estimatedSchedule.replace(/_/g, " ")}</p>)}
                {req.preferredTime && (<p>
            ...
            <div className="max-h-64 space-y-3 overflow-y-auto pr-1">
              {messages.length === 0 ? (<p className="text-sm text-[#475569]">
                ...
                return (<div key={message.id} className={`flex ${fromCustomer ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[85%] rounded-[18px] px-4 py-3 text-sm ${fromCustomer
                        ? "bg-[#2563EB] text-white"
...
function FilterTabs({ active, counts, onChange, }) {
    return (<div className="flex flex-wrap gap-2">
      {FILTERS.map((f) => {
            ...
            return (<button key={f.key} type="button" onClick={() => onChange(f.key)} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold transition-colors ${isActive
            ...
            {f.label}
            <span className={`inline-flex min-w-[20px] items-center justify-center rounded-full px-1.5 text-xs font-bold ${isActive
...
function EmptyState({ filter }) {
    ...
    return (<div className="flex flex-col items-center space-y-3 rounded-[24px] border border-[#E2E8F0] bg-white p-12 text-center">
      <div className="flex size-14 items-center justify-center rounded-full bg-[#EFF6FF]">
        {filter === "ALL" ? (<Wrench className="size-7 text-[#2563EB]"/>) : (<Inbox className="size-7 text-[#2563EB]"/>)}
      </div>
      <p className="text-lg font-semibold text-[#0F172A]">{title}</p>
      <p className="max-w-sm text-sm text-[#94A3B8]">{body}</p>
      <Link to="/get-quotes" className="mt-2 rounded-[14px] bg-[#2563EB] px-6 py-2.5 text-sm font-bold text-white hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
        Get Help
      </Link>
    </div>);
}
...
function CardSkeleton() {
    return (<div className="animate-pulse overflow-hidden rounded-[24px] border border-[#E2E8F0] border-l-4 border-l-[#E2E8F0] bg-white p-5">
      <div className="flex items-start justify-between gap-3">
    ...
    return (<div className="min-h-screen bg-[#F8FAFC]">
      ...
      <div className="sticky top-0 z-20 border-b border-[#E2E8F0] bg-[#F8FAFC]/90 backdrop-blur">
        <div className="mx-auto flex max-w-5xl flex-col justify-between gap-4 px-6 py-5 sm:flex-row sm:items-center">
          ...
          <Link to="/get-quotes" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-[14px] bg-[#2563EB] px-5 py-3 text-sm font-bold text-white hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
      ...
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        ...
        {!error && (<FilterTabs active={filter} counts={counts} onChange={setFilter}/>)}
        ...
        {!isLoading && !error && visible.length > 0 && (<div className="space-y-4">
            {visible.map((req) => (<RequestCard key={req.id} req={req}/>))}
          </div>)}
      </div>
    </div>);
}
//# sourceMappingURL=MyRequestsPage.jsx.map