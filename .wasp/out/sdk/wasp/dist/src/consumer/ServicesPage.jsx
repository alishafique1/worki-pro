...
export default function ServicesPage() {
    ...
    const filteredLive = useMemo(() => search.trim()
        ? liveCategories.filter((c) => c.name.toLowerCase().includes(search.toLowerCase()) ||
            c.description.toLowerCase().includes(search.toLowerCase()))
        : liveCategories, [search]);
    const filteredSoon = useMemo(() => search.trim()
        ? comingSoonCategories.filter((c) => c.name.toLowerCase().includes(search.toLowerCase()))
        : comingSoonCategories, [search]);
    const handleSearch = (e) => {
        e.preventDefault();
        if (filteredLive.length === 1)
            navigate(filteredLive[0].href);
    };
    return (<div className="bg-[#F8FAFC] min-h-screen">
      ...
      <div className="bg-white border-b border-[#E2E8F0]">
        <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          <form onSubmit={handleSearch} className="relative max-w-2xl">
            ...
            <Link to={`/get-quotes${search ? `?q=${encodeURIComponent(search)}` : ''}`} className="absolute right-2 top-1/2 -translate-y-1/2 px-5 py-2.5 bg-[#2563EB] text-white font-bold rounded-full text-sm hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
              Get Help
            </Link>
          </form>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        {filteredLive.length > 0 && (<section className="mb-10">
        ...
        {search && filteredLive.length === 0 && (<div className="bg-white border border-[#E2E8F0] rounded-[24px] p-8 text-center mb-10">
            <p className="text-[#475569] mb-4">
              No live category matches "<strong>{search}</strong>" yet — but our pros can still help.
            </p>
            <Link to={`/get-quotes?q=${encodeURIComponent(search)}`} className="inline-flex items-center gap-2 px-6 py-3 bg-[#2563EB] text-white font-bold rounded-full text-sm hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
              Get Help with this →
            </Link>
          </div>)}

        {/* Coming soon — compact chips */}
        {filteredSoon.length > 0 && (<section>
            <h2 className="text-xs font-bold text-[#94A3B8] uppercase tracking-widest mb-4">
              Coming soon
            </h2>
            <div className="flex flex-wrap gap-3">
              {filteredSoon.map((cat) => (<div key={cat.slug} className="flex items-center gap-2 px-4 py-2.5 bg-white border border-[#E2E8F0] rounded-full opacity-60">
        ...
        {!search && (<div className="mt-14 bg-[#0F172A] rounded-[28px] p-10 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
            ...
            <Link to="/get-quotes" className="shrink-0 px-7 py-4 bg-[#2563EB] text-white font-bold rounded-full text-sm hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
              Get Help →
            </Link>
          </div>)}
      </div>
...
//# sourceMappingURL=ServicesPage.jsx.map