export default function StepCategory({ state, update, onNext }) {
    return (<div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {HARDCODED_CATEGORIES.map(cat => (<button key={cat.slug} type="button" onClick={() => { update({ categoryId: cat.id, categorySlug: cat.slug, categoryName: cat.name }); onNext(); }} className={`border-2 rounded-2xl p-5 text-left transition-all cursor-pointer ${state.categorySlug === cat.slug ? 'border-[#2563EB] bg-[#EFF6FF]' : 'border-[#E2E8F0] bg-white hover:border-[#94A3B8]'}`}>
...
//# sourceMappingURL=StepCategory.jsx.map