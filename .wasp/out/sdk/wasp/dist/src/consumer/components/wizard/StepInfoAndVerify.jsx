...
export default function StepInfoAndVerify({ state, update, onBack, onSuccess, setError }) {
    ...
    async function handleSubmit() {
        ...
        if (!CA_POSTAL.test(trimmedPostal)) {
            setFieldError('Enter a valid Canadian postal code (e.g. L9T 2X5).');
            return;
        }
        if (!firstName.trim()) {
    ...
    async function sendCode(emailAddr) {
        ...
        <form onSubmit={handleVerify} className="space-y-6">
          <div>
            ...
            <div className="flex gap-2 justify-between" onPaste={handlePaste}>
              {code.map((digit, i) => (<input key={i} ref={el => { inputRefs.current[i] = el; }} type="text" inputMode="numeric" maxLength={1} value={digit} onChange={e => handleOtpInput(i, e.target.value)} onKeyDown={e => handleOtpKeyDown(i, e)} className="w-12 h-14 text-center text-2xl font-black bg-white border-2 rounded-xl transition-colors focus:outline-none focus:border-[#2563EB]" style={{ borderColor: digit ? '#2563EB' : '#E2E8F0' }}/>))}
            </div>
          </div>
    ...
    return (<div>
      ...
      <div className="space-y-4">
        <div>
          ...
          <input type="text" className={`${inputClass} uppercase`} placeholder="L9T 2X5" value={postal} onChange={e => { setPostal(e.target.value); clearFieldError(); }} maxLength={7}/>
      ...
      <div className="flex justify-between mt-8">
        ...
        <button type="button" onClick={handleSubmit} disabled={isLoading} className="px-8 py-3 bg-[#2563EB] text-white font-bold rounded-full hover:bg-[#1D4ED8] transition-colors disabled:opacity-50">
          {isLoading ? 'Sending…' : 'Get free quotes →'}
        </button>
      </div>
    </div>);
}
//# sourceMappingURL=StepInfoAndVerify.jsx.map