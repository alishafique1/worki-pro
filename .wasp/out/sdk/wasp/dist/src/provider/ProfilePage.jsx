...
export default function ProviderProfilePage() {
    ...
    const handlePhotoUpload = async (file) => {
        ...
        try {
            ...
            Object.entries(s3UploadFields).forEach(([k, v]) => form.append(k, v));
    ...
    const handleSetProfilePhoto = async (url) => {
        setPhotoMsg(null);
        try {
    ...
    return (<div className="p-8 max-w-3xl mx-auto space-y-6">
      ...
      {profile && !isEditing && (<>
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-[24px] p-6 space-y-4">
            ...
            {photoMsg && (<p className="text-sm text-[#475569] bg-[#F8FAFC] border border-[#E2E8F0] rounded-[12px] px-4 py-2">
                {photoMsg}
              </p>)}
            ...
            <div>
              ...
              <button onClick={() => fileInputRef.current?.click()} disabled={isUploading || portfolio.length >= 12} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-[14px] bg-[#2563EB] text-white text-sm font-semibold hover:bg-[#1D4ED8] transition-colors disabled:opacity-50 shadow-[0_8px_24px_rgba(37,99,235,0.2)]">
            ...
            {/* Thumbnail grid */}
            {portfolio.length > 0 && (<div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {portfolio.map((item) => (<div key={item.url} className="relative group rounded-[14px] overflow-hidden border border-[#E2E8F0] aspect-square bg-[#F8FAFC]">
                    <img src={item.url} alt={item.caption ?? 'Business photo'} className="w-full h-full object-cover"/>
                    {/* Overlay actions */}
                    <div className="absolute inset-0 bg-[#0F172A]/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-between p-2 gap-1">
                      <button onClick={() => handleSetProfilePhoto(item.url)} title="Set as profile photo" className="flex items-center gap-1 px-2 py-1 rounded-[8px] bg-[#F59E0B] text-white text-xs font-semibold hover:bg-amber-600 transition-colors">
                        <Star className="size-3"/>
                        Profile
                      </button>
                      <button onClick={() => handleRemovePhoto(item.url)} title="Remove photo" className="p-1.5 rounded-[8px] bg-red-500 text-white hover:bg-red-600 transition-colors">
                        <Trash2 className="size-3.5"/>
                      </button>
                    </div>
                    {/* Profile photo indicator */}
                    {item.url === formData.profilePhotoUrl && (<div className="absolute top-1.5 left-1.5">
            ...
            {portfolio.length === 0 && !isUploading && (<p className="text-[#94A3B8] text-sm">No photos yet. Upload your first business photo above.</p>)}
          </div>
        </>)}
