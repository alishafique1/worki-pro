...
export default function ProviderDashboardPage() {
    ...
    return (<div className="p-8 max-w-7xl mx-auto space-y-10 bg-[#F8FAFC] min-h-screen">
      ...
      {acceptError && (<div className="rounded-[14px] bg-red-500/10 border border-red-400/30 px-5 py-3 text-sm text-red-600 font-medium">
          {acceptError}
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
            ...
            { label: "Upcoming Bookings", value: appts?.filter((a) => a.scheduledAt && new Date(a.scheduledAt) >= new Date()).length ?? 0, icon: <CalendarClock className="size-5 text-[#3B82F6]"/>, color: "blue" },
      ...
      <div className="bg-white rounded-[24px] border border-[#E2E8F0] p-8 shadow-sm">
        ...
        {!profileLoading && (() => {
            ...
            return (<div className="flex flex-wrap gap-2 mt-2">
              {services.slice(0, 6).map((svc) => (<span key={svc.id} className="px-3 py-1.5 bg-[#EFF6FF] border border-[#BFDBFE] rounded-full text-xs font-medium text-[#0F172A]">
                  {svc.name}
                  {svc.price && <span className="text-[#2563EB] ml-1 font-bold">${svc.price}</span>}
                </span>))}
              {services.length > 6 && (<span className="px-3 py-1.5 text-xs text-[#94A3B8]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-[24px] border border-[#E2E8F0] p-8 shadow-sm">
          <div className="flex justify-between items-center gap-4 mb-6">
            ...
            <Link to="/provider/leads" className="text-sm font-bold text-[#2563EB] hover:underline">
              View all →
            </Link>
          </div>

          {leadsLoading ? (<div className="animate-pulse h-32 bg-[#EFF6FF] rounded-[14px]"></div>) : leadsError ? (<div className="rounded-[14px] border border-[#E2E8F0] bg-[#F8FAFC] p-6">
              <p className="font-bold text-[#0F172A]">Leads could not load</p>
              <p className="mt-2 text-sm text-[#475569]">
                Refresh the page or open the lead inbox to try again.
              </p>
              <Link to="/provider/leads" className="mt-3 inline-block text-sm font-bold text-[#2563EB] hover:underline">
                Open lead inbox →
              </Link>
        ...
        <div className="bg-white rounded-[24px] border border-[#E2E8F0] p-8 shadow-sm">
          <div className="mb-6 flex items-center justify-between gap-4">
            ...
            <Link to="/provider/appointments" className="text-sm font-bold text-[#2563EB] hover:underline">
              Manage all →
            </Link>
          </div>

          {apptsLoading ? (<div className="animate-pulse h-32 bg-[#EFF6FF] rounded-[14px]"></div>) : apptsError ? (<div className="rounded-[14px] border border-[#E2E8F0] bg-[#F8FAFC] p-6">
              <p className="font-bold text-[#0F172A]">Bookings could not load</p>
              <p className="mt-2 text-sm text-[#475569]">
                Refresh the page or open bookings to try again.
              </p>
              <Link to="/provider/appointments" className="mt-3 inline-block text-sm font-bold text-[#2563EB] hover:underline">
                Open bookings →
              </Link>
            </div>) : (<div className="space-y-4">
