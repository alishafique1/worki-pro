...
import { MapPin, AlertTriangle, Clock, Search, CheckCircle2 } from 'lucide-react';
...
export default function ProviderLeadsPage() {
    ...
    return (<div className="p-6 max-w-5xl mx-auto space-y-8">
      ...
      <div className="flex flex-wrap gap-3 p-4 rounded-[18px] border border-[#E2E8F0] bg-white">
        <div>
          ...
          <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="px-3 py-2 rounded-[12px] border border-[#E2E8F0] bg-[#F8FAFC] text-sm font-medium focus:outline-none focus:border-[#2563EB] focus:ring-1 focus:ring-[#2563EB]/30">
            <option value="">All categories</option>
            {categories?.map((cat) => (<option key={cat.slug} value={cat.slug}>
                {cat.name}
              </option>))}
          </select>
        </div>
        ...
        <div>
          ...
          <select value={urgencyFilter} onChange={(e) => setUrgencyFilter(e.target.value)} className="px-3 py-2 rounded-[12px] border border-[#E2E8F0] bg-[#F8FAFC] text-sm font-medium focus:outline-none focus:border-[#2563EB] focus:ring-1 focus:ring-[#2563EB]/30">
        ...
        {(categoryFilter || urgencyFilter) && (<div className="flex items-end">
            <button onClick={() => { setCategoryFilter(""); setUrgencyFilter(""); }} className="px-3 py-2 text-sm font-bold text-[#475569] hover:text-[#0F172A] transition-colors">
              Clear filters
            </button>
          </div>)}
      </div>
      ...
      {/* Error banner */}
      {claimError && (<div className="p-4 rounded-[14px] border border-red-500/30 bg-red-500/10 text-red-400 text-sm font-medium">
          {claimError}
        </div>)}
      ...
      <div className="space-y-4">
        {isLoading && (<>
            {[0, 1, 2].map((i) => (<div key={i} className="rounded-[18px] border border-[#E2E8F0] bg-white p-6 animate-pulse">
        ...
        {leads?.map((lead) => {
            ...
            return (<div key={lead.id} className="rounded-[18px] border border-[#E2E8F0] bg-white shadow-sm p-6 transition-all hover:border-[#BFDBFE]">
              {/* Top row */}
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="flex flex-wrap items-center gap-2">
                  {lead.serviceCategory && (<span className="px-3 py-1 rounded-full border text-xs font-bold border-[#E2E8F0] text-[#475569]">
                      {lead.serviceCategory.name}
                    </span>)}
                  <span className={`px-3 py-1 rounded-full border text-xs font-bold ${urgencyInfo.color}`}>
                    {urgencyInfo.label}
                  </span>
                  {isClaimed && (<span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-bold border-green-200 text-green-700 bg-green-50">
              ...
              {/* CTA */}
              {!isClaimed ? (<div className="flex items-center gap-3">
                  <button onClick={() => setConfirmLead({ id: lead.id, fee: leadFee })} disabled={isClaiming} className="px-6 py-2.5 bg-[#2563EB] text-white text-sm font-black rounded-[14px] hover:bg-[#1D4ED8] disabled:opacity-50 transition-all shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
                    {isClaiming ? "Claiming…" : "Claim this lead"}
                  </button>
                  <span className="text-xs text-[#94A3B8]">
                    Reveal customer contact · ${leadFee} lead fee
                  </span>
                </div>) : (<Link to={`/provider/requests/${lead.id}/messages`} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-[14px] border border-[#BFDBFE] text-[#2563EB] text-sm font-bold hover:bg-[#EFF6FF] transition-all">
      ...
      <Dialog open={!!confirmLead} onOpenChange={(open) => { if (!open)
        setConfirmLead(null); }}>
        <DialogContent className="bg-white border-[#E2E8F0] rounded-[18px]">
          ...
          <DialogFooter className="gap-2">
            <button onClick={() => setConfirmLead(null)} className="px-5 py-2.5 rounded-[14px] border border-[#E2E8F0] text-[#475569] text-sm font-bold hover:border-[#2563EB] transition-colors">
              Cancel
            </button>
            <button onClick={() => confirmLead && handleClaim(confirmLead.id)} className="px-6 py-2.5 bg-[#2563EB] text-white text-sm font-black rounded-[14px] hover:bg-[#1D4ED8] transition-all">
...
//# sourceMappingURL=LeadsPage.jsx.map