...
import { assertTransition, canTransition, CLAIMABLE_STATUSES, } from "../shared/requestStatusMachine";
...
export const getProviderLeads = async (args, context) => {
...
export const getProviderAppointments = async (args, context) => {
    const provider = await requireProvider(context);
...
export const getProviderProfile = async (args, context) => {
    if (!context.user)
        throw new HttpError(401);
    const provider = await context.entities.Provider.findUnique({
        where: { userId: context.user.id },
...
export const getProviderFees = async (args, context) => {
    const provider = await requireProvider(context);
...
export const acceptServiceRequest = async ({ requestId }, context) => {
    const provider = await requireProvider(context);
...
export const createProviderProfile = async (args, context) => {
    if (!context.user)
        throw new HttpError(401);
...
export const submitProviderApplication = async (args, context) => {
    if (!context.user)
        throw new HttpError(401);
...
export const updateProviderProfile = async (args, context) => {
    const provider = await requireProvider(context);
    ...
    if (args.contactName !== undefined)
        data.contactName = args.contactName;
    if (args.phone !== undefined)
        data.phone = args.phone.trim();
    if (args.email !== undefined)
        data.email = args.email;
    if (args.website !== undefined)
        data.website = args.website;
    if (args.serviceAreas !== undefined)
        data.serviceAreas = args.serviceAreas;
    if (args.calComUsername !== undefined)
    ...
    if (args.bio !== undefined)
        data.bio = args.bio || null;
    if (args.profilePhotoUrl !== undefined)
        data.profilePhotoUrl = args.profilePhotoUrl || null;
    if (args.portfolioJson !== undefined)
        data.portfolioJson = args.portfolioJson;
    if (args.accreditationsJson !== undefined)
        data.accreditationsJson = args.accreditationsJson;
    if (args.responseTimeMins !== undefined)
        data.responseTimeMins = args.responseTimeMins || null;
    // Credentials — explicit empty string clears the value (owner can retract).
    if (args.licenceNumber !== undefined)
        data.licenceNumber = args.licenceNumber.trim() || null;
    if (args.insuranceUrl !== undefined)
        data.insuranceUrl = args.insuranceUrl.trim() || null;
    if (args.wsibClearanceNumber !== undefined)
        data.wsibClearanceNumber = args.wsibClearanceNumber.trim() || null;
    return context.entities.Provider.update({
        where: { id: provider.id },
        data,
    });
};
export const markJobCompleted = async ({ appointmentId }, context) => {
    const provider = await requireProvider(context);
    const appt = await context.entities.Appointment.findUnique({
        where: { id: appointmentId },
    });
    if (!appt || appt.providerId !== provider.id)
    ...
    const needsRequestUpdate = !!serviceRequest && serviceRequest.status !== "COMPLETED";
...
export const updateProviderServices = async ({ services }, context) => {
    const provider = await requireProvider(context);
    return context.entities.Provider.update({
        where: { id: provider.id },
...
export const updateProviderAppointment = async (args, context) => {
    const provider = await requireProvider(context);
    ...
    if (serviceRequest) {
        const target = updated.status === "CONFIRMED"
            ? "BOOKED"
        ...
        if (target && canTransition(serviceRequest.status, target)) {
            ...
            console.warn(`[updateProviderAppointment] Skipping request status ${serviceRequest.status}→${target} for ${updated.serviceRequestId} — invalid transition`);
        }
    }
    return updated;
};
export const sendProviderMessage = async ({ requestId, body }, context) => {
    const provider = await requireProvider(context);
...
export const disputeLeadFee = async ({ feeId, reason, note }, context) => {
    const provider = await requireProvider(context);
    ...
    if (Date.now() - new Date(fee.createdAt).getTime() > windowMs) {
        throw new HttpError(400, `Lead fees can only be disputed within ${DISPUTE_WINDOW_DAYS} days.`);
    }
...
export const getPublicProvider = async ({ slug }, context) => {
    ...
    if (!provider)
        return null;
    const { licenceNumber, tssaRegistrationNumber, insuranceUrl, insuranceStatus, wsibClearanceNumber, ...publicFields } = provider;
    return {
        ...publicFields,
