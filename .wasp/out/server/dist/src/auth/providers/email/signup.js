import { createProviderId, createUser, deleteUserByAuthId, doFakeWork, findAuthIdentity, getProviderDataWithPassword, rethrowPossibleAuthError, sanitizeAndSerializeProviderData, validateAndGetUserFields, } from 'wasp/auth/utils';
import { ensurePasswordIsPresent, ensureValidEmail, ensureValidPassword, } from 'wasp/auth/validation';
import { HttpError } from 'wasp/server';
import { createEmailVerificationLink, isEmailResendAllowed, sendEmailVerificationEmail, } from 'wasp/server/auth/email/utils';
import { onAfterSignupHook, onBeforeSignupHook } from '../../hooks.js';
export function getSignupRoute({ userSignupFields, fromField, clientRoute, getVerificationEmailContent, isEmailAutoVerified, }) {
    return async function signup(req, res) {
        ...
        if (existingAuthIdentity) {
            const providerData = getProviderDataWithPassword(existingAuthIdentity.providerData);
        ...
        try {
            ...
            const user = await createUser(providerId, newUserProviderData, 
