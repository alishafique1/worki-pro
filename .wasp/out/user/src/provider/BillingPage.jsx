    ...
    return (<div className="p-8 max-w-6xl mx-auto space-y-8 bg-[#F8FAFC] min-h-screen">
      ...
      <div className="bg-white p-6 rounded-[14px] border border-[#E2E8F0] shadow-sm">
        ...
        <div className="space-y-4">
          ...
          {fees?.map((fee) => (<div key={fee.id} className="flex justify-between items-center p-4 border border-[#E2E8F0] rounded-[14px] bg-[#F8FAFC]">
              <div>
              ...
              <div className="text-right">
                ...
                <p className={`text-xs font-semibold px-2 py-0.5 rounded-full border inline-block ${fee.status === 'PENDING'
                ? 'bg-[#EFF6FF] text-[#2563EB] border-[#BFDBFE]'
                : 'bg-[#F0FDF4] text-[#15803D] border-green-200'}`}>{fee.status}</p>
              </div>
            </div>))}
        </div>
      </div>
