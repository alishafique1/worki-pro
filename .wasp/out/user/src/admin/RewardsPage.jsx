...
export default function AdminRewardsPage() {
    ...
    return (<div className="p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="bg-white p-6 rounded-[14px] border border-[#E2E8F0]">
        {isLoading ? (<p className="text-[#475569]">Loading...</p>) : rewards?.length === 0 ? (<div className="py-12 text-center text-[#475569]">
            ...
            <tbody>
              {rewards?.map((reward) => (<tr key={reward.id} className="border-b border-[#E2E8F0] last:border-0">
                  ...
                  <td className="py-3 space-y-1">
                    {approvedIds.has(reward.id) ? (<span className="text-xs text-[#22C55E] font-medium">✓ Approved</span>) : rejectedIds.has(reward.id) ? (<span className="text-xs text-red-600 font-medium">✗ Rejected</span>) : (<>
                        ...
                        {errorMap[reward.id] && (<p className="text-xs text-red-600">{errorMap[reward.id]}</p>)}
                      </>)}
                  </td>
                </tr>))}
            </tbody>
