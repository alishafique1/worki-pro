...
export default function StepOtp({ state, onBack, onVerified, externalError }) {
    ...
    return (<div>
      ...
      {sendState === 'sending' && (<p className="text-sm text-[#94A3B8] mb-4">Sending your code…</p>)}
      {sendError && (<p className="text-sm text-red-500 mb-4 bg-red-500/10 border border-red-500/20 rounded-[10px] px-4 py-3">
          {sendError}
        </p>)}

      <div onPaste={handlePaste} className="flex gap-2 justify-between mb-6" role="group" aria-label="Verification code">
        {code.map((digit, i) => (<input key={i} ref={(el) => { inputRefs.current[i] = el; }} type="text" inputMode="numeric" maxLength={1} value={digit} aria-label={`Digit ${i + 1}`} onChange={(e) => handleChange(i, e.target.value)} onKeyDown={(e) => handleKeyDown(i, e)} className="w-12 h-14 sm:w-14 sm:h-16 text-center text-2xl font-black bg-white border-2 rounded-xl transition-colors focus:outline-none focus:border-[#2563EB]" style={{ borderColor: digit ? '#2563EB' : '#E2E8F0' }} disabled={sendState !== 'sent'}/>))}
      </div>

      {(verifyError || externalError) && (<p className="text-sm text-red-500 mb-4 bg-red-500/10 border border-red-500/20 rounded-[10px] px-4 py-3">
          {verifyError || externalError}
        </p>)}

      <button type="button" onClick={verify} disabled={verifyState === 'verifying' || code.join('').length !== 6 || sendState !== 'sent'} className="w-full py-3 bg-[#2563EB] text-white font-bold rounded-xl hover:bg-[#1D4ED8] transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
        {verifyState === 'verifying' ? 'Verifying…' : 'Verify & get matched →'}
      </button>

      <p className="text-center text-sm text-[#475569] mt-4">
        Didn't get it?{' '}
        {resendIn > 0 ? (<span>Resend in {resendIn}s</span>) : (<button type="button" onClick={send} disabled={sendState === 'sending'} className="text-[#2563EB] font-semibold hover:underline disabled:opacity-50">
            Resend code
          </button>)}
      </p>
