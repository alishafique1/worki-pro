...
import { useQuery, getMyRewardAccount, getMyReferral, redeemPoints, } from 'wasp/client/operations';
import { Gift, Sparkles, Crown, Star, TrendingUp, FileText, CalendarCheck, CheckCircle2, Users, Copy, Check, Mail, Wallet, ArrowRight, Share2, } from 'lucide-react';
...
function HeroSkeleton() {
    return (<div className="rounded-[24px] bg-white border border-[#E2E8F0] p-8 animate-pulse">
...
function RowSkeleton() {
    return (<div className="flex items-center justify-between py-3 animate-pulse">
...
export default function RewardsPage() {
    ...
    const filteredTxns = txns.filter((tx) => historyTab === 'all'
        ? true
        : historyTab === 'earned'
            ? tx.points > 0
            : tx.points < 0);
    async function handleRedeem(e) {
        ...
        try {
            ...
            setSuccess(`Successfully redeemed ${effectivePoints.toLocaleString()} points. Check your email!`);
            setEmail('');
            setCustomPoints('');
    ...
    return (<div className="min-h-screen bg-[#F8FAFC]">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl text-[#0F172A] font-black tracking-tight" style={{ fontFamily: 'Fraunces, serif' }}>
              Rewards Wallet
            </h1>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            {isLoading ? (<HeroSkeleton />) : (<div className="relative overflow-hidden rounded-[24px] p-8 text-white shadow-[0_20px_50px_-12px_rgba(245,158,11,0.45)]" style={{
                ...
                <div className="relative">
                  ...
                  <div className="mt-5 flex items-end gap-3">
                    <span className="text-6xl leading-none font-black tracking-tight" style={{ fontFamily: 'Fraunces, serif' }}>
                ...
                : 'You can redeem a 10,000 pts reward!'}
                    ...
                    <div className="h-2.5 w-full bg-white/30 rounded-full overflow-hidden">
                      <div className="h-full bg-white rounded-full transition-all duration-700" style={{ width: `${redemptionProgress}%` }}/>
                    </div>
                  </div>
                </div>
              </div>)}
          ...
          <div className="rounded-[24px] bg-white border border-[#E2E8F0] p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {next && (<span className="text-xs text-[#475569]">
            ...
            const Icon = lvl.icon;
            return (<div key={lvl.key} className={`flex items-center gap-3 rounded-[14px] px-3 py-2.5 transition-colors ${isCurrent
                    ? 'bg-[#FEF3C7] border border-[#F59E0B]'
                    : 'border border-transparent'}`}>
                    <div className={`h-8 w-8 rounded-[10px] flex items-center justify-center shrink-0 ${isCurrent
                    ...
                    <div className="min-w-0 flex-1">
                      <p className={`text-sm font-bold truncate ${isCurrent ? 'text-[#92400E]' : 'text-[#0F172A]'}`}>
                        {lvl.label}
                      </p>
                    ...
                    {isCurrent && (<span className="text-[10px] font-bold uppercase tracking-wider text-[#92400E]">
                        You
                      </span>)}
                  </div>);
        })}
            ...
            {next && (<div className="mt-4 pt-4 border-t border-[#E2E8F0]">
        ...
        <div className="rounded-[24px] bg-white border border-[#E2E8F0] p-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {EARN_ACTIONS.map((a) => {
            const Icon = a.icon;
            return (<div key={a.label} className="group rounded-[20px] border border-[#E2E8F0] p-5 hover:border-[#2563EB] hover:shadow-[0_8px_24px_-12px_rgba(37,99,235,0.3)] transition-all">
                  <div className="h-11 w-11 rounded-[14px] flex items-center justify-center mb-4" style={{ backgroundColor: `${a.color}1A` }}>
                    <Icon className="h-5 w-5" style={{ color: a.color }}/>
                  </div>
                  <p className="text-2xl font-black tracking-tight" style={{ color: a.color, fontFamily: 'Fraunces, serif' }}>
                    {a.pts}
                  </p>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-[24px] bg-white border border-[#E2E8F0] p-6">
            ...
            return (<button key={p.points} type="button" onClick={() => {
                    setSelectedPoints(p.points);
                    setCustomPoints('');
                }} className={`rounded-[14px] border px-3 py-3 text-center transition-all ${active
                    ...
                    : 'bg-white text-[#0F172A] border-[#E2E8F0] hover:border-[#2563EB]'}`}>
                      <span className="block text-lg font-black" style={{ fontFamily: 'Fraunces, serif' }}>
                        {p.label}
                      </span>
                      <span className={`block text-[11px] font-semibold ${active ? 'text-white/80' : 'text-[#475569]'}`}>
              ...
              <div>
                ...
                <input type="number" min={500} step={500} value={customPoints} onChange={(e) => setCustomPoints(e.target.value)} placeholder="e.g. 1500" className="w-full bg-white border border-[#E2E8F0] rounded-[12px] px-3.5 py-2.5 text-sm text-[#0F172A] focus:outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/15"/>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="w-full bg-white border border-[#E2E8F0] rounded-[12px] pl-9 pr-3.5 py-2.5 text-sm text-[#0F172A] focus:outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/15"/>
                </div>
              </div>

              {error && (<p className="text-sm text-[#991B1B] bg-[#FEE2E2] rounded-[12px] px-3 py-2">
                  {error}
                </p>)}
              {success && (<p className="text-sm text-[#166534] bg-[#DCFCE7] rounded-[12px] px-3 py-2">
                  {success}
                </p>)}

              <button type="submit" disabled={submitting || effectivePoints > balance} className="w-full inline-flex items-center justify-center gap-2 py-3 rounded-[14px] bg-[#2563EB] text-white font-bold text-sm hover:bg-[#1D4ED8] disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-[0_12px_28px_-10px_rgba(37,99,235,0.55)]">
                {submitting
            ? 'Processing…'
            : effectivePoints >= 500
              ...
              {effectivePoints > balance && (<p className="text-xs text-[#475569] text-center">
          ...
          <div className="rounded-[24px] border border-[#E2E8F0] bg-white p-6 flex flex-col">
            ...
            <div className="rounded-[20px] bg-[#F8FAFC] border border-[#E2E8F0] p-5">
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={copyCode} disabled={!referralCode} className={`h-[50px] w-[50px] shrink-0 rounded-[12px] flex items-center justify-center font-bold transition-all disabled:opacity-40 ${copied
            ? 'bg-[#22C55E] text-white'
            : 'bg-[#2563EB] text-white hover:bg-[#1D4ED8]'}`} aria-label="Copy referral code">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <a href={shareUrl
            ? `sms:?&body=${encodeURIComponent(`Get vetted home pros on The Helper — use my code ${referralCode}: ${shareUrl}`)}`
            : '#'} className="inline-flex items-center justify-center gap-2 rounded-[14px] border border-[#E2E8F0] px-4 py-2.5 text-sm font-semibold text-[#0F172A] hover:border-[#2563EB] transition-colors">
              ...
              <a href={shareUrl
            ? `mailto:?subject=${encodeURIComponent('Try The Helper')}&body=${encodeURIComponent(`Get vetted home pros on The Helper — use my code ${referralCode}: ${shareUrl}`)}`
            : '#'} className="inline-flex items-center justify-center gap-2 rounded-[14px] border border-[#E2E8F0] px-4 py-2.5 text-sm font-semibold text-[#0F172A] hover:border-[#2563EB] transition-colors">
        ...
        <div className="rounded-[24px] bg-white border border-[#E2E8F0] overflow-hidden">
          <div className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-[#E2E8F0] px-6 py-4 flex flex-wrap items-center gap-3 justify-between">
            ...
            <div className="flex items-center gap-1 bg-[#F8FAFC] border border-[#E2E8F0] rounded-[12px] p-1">
              {['all', 'earned', 'redeemed'].map((tab) => (<button key={tab} onClick={() => setHistoryTab(tab)} className={`px-3 py-1.5 rounded-[9px] text-xs font-bold capitalize transition-colors ${historyTab === tab
          ...
          <div className="px-6 py-2">
            {isLoading ? (<div className="divide-y divide-[#E2E8F0]">
                {[0, 1, 2].map((i) => (<RowSkeleton key={i}/>))}
              </div>) : !filteredTxns.length ? (<div className="py-14 flex flex-col items-center text-center">
                ...
                <a href="/request" className="mt-5 inline-flex items-center gap-2 rounded-[14px] bg-[#2563EB] text-white px-5 py-2.5 text-sm font-bold hover:bg-[#1D4ED8] transition-colors">
                  Start a request <ArrowRight className="h-4 w-4"/>
                </a>
              </div>) : (<div className="divide-y divide-[#E2E8F0]">
                {filteredTxns.map((tx) => {
                const positive = tx.points > 0;
                const label = TYPE_LABELS[tx.type] ?? tx.type;
                return (<div key={tx.id} className="flex items-center justify-between py-3.5 -mx-2 px-2 rounded-[12px] hover:bg-[#F8FAFC] transition-colors">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`h-9 w-9 rounded-[12px] flex items-center justify-center shrink-0 ${positive ? 'bg-[#DCFCE7]' : 'bg-[#FEF3C7]'}`}>
                          {positive ? (<TrendingUp className="h-4 w-4 text-[#166534]"/>) : (<Gift className="h-4 w-4 text-[#92400E]"/>)}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-[#0F172A] truncate">{label}</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${STATUS_BADGE[tx.status] ?? 'bg-[#F1F5F9] text-[#475569]'}`}>
                      ...
                      <span className={`text-sm font-black tabular-nums shrink-0 ${positive ? 'text-[#166534]' : 'text-[#991B1B]'}`}>
                        {positive ? '+' : '−'}
        ...
        {/* ── REDEMPTION HISTORY ──────────────────────────────── */}
        {!!data?.redemptions?.length && (<div className="rounded-[24px] bg-white border border-[#E2E8F0] p-6">
            <h2 className="text-lg font-bold text-[#0F172A] mb-4">Gift card redemptions</h2>
            <div className="divide-y divide-[#E2E8F0]">
              {data.redemptions.map((r) => (<div key={r.id} className="flex items-center justify-between py-3.5 -mx-2 px-2 rounded-[12px] hover:bg-[#F8FAFC] transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${STATUS_BADGE[r.status] ?? 'bg-[#F1F5F9] text-[#475569]'}`}>
