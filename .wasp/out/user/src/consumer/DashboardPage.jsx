...
export default function DashboardPage() {
    ...
    const filteredRequests = (requests ?? []).filter((req) => {
        if (filter === "ACTIVE")
            return !COMPLETED_OR_DEAD.includes(req.status);
        if (filter === "COMPLETED")
    ...
    return (<div className="min-h-screen bg-[#F8FAFC]">
      <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {!rewardsLoading && (<span className="inline-flex items-center gap-1.5 bg-[#FEF3C7] text-[#92400E] border border-[#FDE68A] rounded-full px-3 py-1.5 text-xs font-bold">
          ...
          <Link to="/account/request-service" className="inline-flex items-center gap-2 px-6 py-3 bg-[#2563EB] text-white font-bold rounded-[14px] hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)] whitespace-nowrap">
        ...
        {requestsLoading ? (<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[0, 1, 2].map((i) => (<div key={i} className="animate-pulse bg-white border border-[#E2E8F0] rounded-[20px] p-6">
            ...
            {[
            ...
            ].map(({ label, value, hint, trend, trendLabel }) => (<div key={label} className="relative bg-white border border-[#E2E8F0] rounded-[20px] p-6 overflow-hidden group hover:shadow-[0_8px_24px_rgba(37,99,235,0.08)] transition-shadow">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`inline-flex items-center gap-1 text-[11px] font-bold rounded-full px-2 py-0.5 ${trend === "up"
                ...
                <div className="mt-3 flex items-end gap-0.5 h-6">
                  {[40, 65, 50, 80, 60, 90, 75].map((h, idx) => (<div key={idx} style={{ height: `${h}%` }} className="flex-1 rounded-sm bg-[#DBEAFE] group-hover:bg-[#BFDBFE] transition-colors"/>))}
                </div>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="sticky top-0 z-10 -mx-2 px-2 py-3 bg-[#F8FAFC]/90 backdrop-blur-sm">
              <div className="flex items-center justify-between gap-4 flex-wrap">
                ...
                <Link to="/account/requests" className="text-sm font-bold text-[#2563EB] hover:underline inline-flex items-center gap-1">
                  View all <ArrowRight className="size-3.5"/>
                </Link>
              </div>
              {/* Filter pills */}
              {!requestsLoading && !requestsError && totalCount > 0 && (<div className="mt-3 inline-flex items-center gap-1 rounded-[14px] bg-white border border-[#E2E8F0] p-1">
                ...
                return (<button key={pill.key} type="button" onClick={() => setFilter(pill.key)} className={`inline-flex items-center gap-1.5 rounded-[10px] px-4 py-1.5 text-sm font-semibold transition-colors ${active
                        ? "bg-[#2563EB] text-white shadow-[0_4px_12px_rgba(37,99,235,0.25)]"
                        : "text-[#475569] hover:bg-[#F8FAFC]"}`}>
                        {pill.label}
                        <span className={`text-xs font-bold rounded-full px-1.5 ${active ? "bg-white/20 text-white" : "bg-[#F1F5F9] text-[#94A3B8]"}`}>
                          {count}
                        </span>
                      </button>);
            })}
            ...
            {!requestsLoading && requestsError && (<div className="rounded-[24px] border border-[#E2E8F0] bg-white p-6">
                ...
                <Link to="/account/requests" className="mt-4 inline-block text-sm font-bold text-[#2563EB] hover:underline">
                  Open request details →
                </Link>
              </div>)}

            {/* Empty state — no requests at all */}
            {!requestsLoading && !requestsError && totalCount === 0 && (<div className="bg-white border border-[#E2E8F0] rounded-[24px] p-12 text-center">
                ...
                <Link to="/account/request-service" className="inline-flex items-center gap-2 px-6 py-3 bg-[#2563EB] text-white font-bold rounded-[14px] hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
            ...
            !requestsError &&
            totalCount > 0 &&
            filteredRequests.length === 0 && (<div className="bg-white border border-[#E2E8F0] rounded-[24px] p-10 text-center">
            ...
            {!requestsLoading && !requestsError && filteredRequests.length > 0 && (<div className="space-y-4">
                ...
                return (<div key={req.id} style={{ borderLeftColor: accent }} className="bg-white border border-[#E2E8F0] border-l-4 rounded-[24px] p-6 hover:border-[#BFDBFE] hover:border-l-4 hover:shadow-[0_8px_24px_rgba(37,99,235,0.08)] transition-all duration-200 group">
                      ...
                      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-[#475569]">
                        ...
                        {provider && (<span className="flex items-center gap-2">
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-3">
                        <Link to={`/account/requests/${req.id}`} className="inline-flex items-center gap-1.5 rounded-[14px] border border-[#E2E8F0] px-4 py-2 text-sm font-bold text-[#475569] hover:border-[#BFDBFE] hover:text-[#2563EB] transition-colors">
                          View details <ArrowRight className="size-3"/>
                        </Link>
                        {!appt?.scheduledAt &&
                        ["ASSIGNED", "ACCEPTED_BY_PROVIDER", "QUALIFIED"].includes(req.status) && (<Link to={`/account/book/${req.id}`} className="inline-flex items-center gap-1.5 rounded-[14px] bg-[#2563EB] px-4 py-2 text-sm font-bold text-white hover:bg-[#1D4ED8] transition-colors shadow-[0_8px_24px_rgba(37,99,235,0.3)]">
          ...
          <div className="space-y-6">
            <div className="bg-white border border-[#E2E8F0] rounded-[24px] p-8 relative overflow-hidden shadow-sm">
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <Link to="/account/rewards" className="text-xs font-bold text-[#2563EB] hover:underline flex items-center gap-1">
              ...
              {rewardsLoading ? (<div className="animate-pulse space-y-3">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="h-3 w-full rounded-full bg-[#F1F5F9] overflow-hidden">
                      <div style={{ width: `${tierProgress}%` }} className="h-full rounded-full bg-gradient-to-r from-[#2563EB] to-[#60A5FA] transition-all duration-500"/>
                    </div>
            ...
            <div className="relative bg-[#0F172A] rounded-[24px] p-7 overflow-hidden">
              ...
              <div className="relative">
                ...
                <Link to="/account/referrals" className="inline-flex items-center gap-2 px-5 py-2.5 bg-white text-[#0F172A] text-sm font-bold rounded-[14px] hover:bg-slate-100 transition-colors">
